...
This module provides functionality to export query results to various formats
including CSV, JSON, XML, Excel, TSV, and HTML using the file manager.
"""
...
class ExportService:
    ...
    Supports exporting to CSV, JSON, XML, Excel, TSV, and HTML formats
    with configurable settings for formatting and encoding.
    """
    ...
    def _prepare_data(self, results: QueryResult, settings: ExportSettings) -> List[Dict[str, Any]]:
        """
        Prepare data for export by applying settings.
    ...
    def _export_to_html(
        ...
        Args:
            data: Data to export
            results: Original query results for metadata
            settings: Export settings
        ...
        html_parts = [
            ...
            "<head>",
            "    <meta charset='UTF-8'>",
            "    <title>Query Results Export</title>",
            "    <style>",
            ...
            "        .metadata { background-color: #e9f4ff; padding: 10px; margin-bottom: 20px; border-radius: 5px; }",
            ...
            "    <div class='metadata'>",
            "        <h3>Query Results</h3>",
            f"        <p><strong>Executed:</strong> {results.executed_at.strftime(settings.date_format)}</p>",
            f"        <p><strong>Connection:</strong> {results.connection_id}</p>",
            f"        <p><strong>Rows:</strong> {results.row_count}</p>",
    ...
    def _export_to_excel(self, data: List[Dict[str, Any]], settings: ExportSettings) -> bytes:
        ...
        try:
            ...
            # Fallback to simple CSV-like format if openpyxl is not available
