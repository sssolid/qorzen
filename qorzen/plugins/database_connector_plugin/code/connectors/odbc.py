...
class ODBCConnector(BaseDatabaseConnector):
    ...
    def __init__(
        ...
        self._is_filemaker = False  # Will detect during connection

        try:
            # Import pyodbc lazily to avoid hard dependency
            import pyodbc
            self._pyodbc = pyodbc
        except ImportError:
            self._logger.error(
        ...
        async with self._connect_lock:
            ...
            try:
                ...
                if self._config.connection_string:
                    ...
                    # Add server and port if provided
                ...
                def connect_sync() -> Tuple[Any, float]:
                    ...
                    # Set read-only mode if possible and required
                ...
                self._logger.info(
                    "Successfully connected to database via ODBC",
                    extra={
    ...
    async def execute_query(
        ...
        try:
            ...
            self._logger.info(
                "Successfully executed query via ODBC",
                extra={
    ...
    async def get_tables(self, schema: Optional[str] = None) -> List[TableMetadata]:
        ...
        def get_tables_sync() -> List[TableMetadata]:
            ...
            try:
                # Get tables using pyodbc
                if self._is_filemaker:
                    # FileMaker requires a specific approach
                    cursor.tables()
                else:
                    # Standard ODBC approach
                    if schema:
                ...
                for row in cursor:
                    ...
                    # Skip system tables and non-table objects
                    if (table_type.upper() in ('TABLE', 'VIEW') and
                        ...
                        try:
                            ...
                            for col_row in col_cursor:
                                ...
                                # Type code isn't always available
                                col_type_code = 0
                                try:
    ...
    def _validate_and_prepare_query(
        ...
        Returns:
            Tuple of (prepared query, table name if detected)
        ...
        if " " not in query:
            ...
            if self._is_filemaker:
                # FileMaker requires specific syntax for layout-based queries
            ...
            # Check for write operations if in read-only mode
        ...
        if limit is not None:
            if "LIMIT" not in query.upper():
                ...
                if self._is_filemaker:
                    # FileMaker uses FETCH FIRST syntax
                    if ";" in query:
                        query = query.rstrip(";")
                    query = f"{query} FETCH FIRST {limit} ROWS ONLY"
                else:
                    # Generic LIMIT syntax for most databases
                    if ";" in query:
                        query = query.rstrip(";")
    ...
    def _prepare_parameters(self, params: Dict[str, Any]) -> List[Any]:
        ...
        Returns:
            List of parameter values in correct order
        """
        # For now, just return the values
        # In the future, we might need more sophisticated parameter handling
        return list(params.values())

    def _get_column_metadata(self, cursor: Any) -> List[ColumnMetadata]:
        """
        Extract column metadata from cursor.
