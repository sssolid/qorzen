...
This module provides the main UI widget that contains all the plugin tabs
including connection management, query editor, results, field mapping,
...
from PySide6.QtWidgets import (
    QWidget, QVBoxLayout, QTabWidget, QMessageBox, QProgressBar,
...
class DatabasePluginWidget(QWidget):
    ...
    - Main tab: Connection management and query editor
    - Results tab: Query results display and export
    - Field Mapping tab: Field mapping management
    - Validation tab: Data validation rules
    - History tab: Historical data tracking
    """
    ...
    def __init__(
        ...
        # Update timer for periodic status updates
    ...
    def _setup_ui(self) -> None:
        ...
        self._field_mapping_tab = FieldMappingTab(
            plugin=self._plugin,
            logger=self._logger,
            concurrency_manager=self._concurrency_manager,
            parent=self
    ...
    def _setup_connections(self) -> None:
        ...
        if not all([self._main_tab, self._results_tab, self._field_mapping_tab,
                    self._validation_tab, self._history_tab]):
            return

        # Connect main tab query execution to results tab
        self._main_tab.query_executed.connect(self._results_tab.show_results)
        self._main_tab.query_executed.connect(lambda: self.switch_to_tab(1))  # Switch to results tab
    ...
    def _update_status(self) -> None:
        ...
        try:
            ...
            # Update status if not currently showing progress
            if self._progress_bar and not self._progress_bar.isVisible():
                ...
                status_text = (
                    f"Connections: {active_connections}/{connections_count} | "
                    f"Saved Queries: {saved_queries}"
                )
