...
This module provides the history tab UI for managing historical data tracking
schedules and viewing collected historical data.
"""
...
from PySide6.QtWidgets import (
    ...
    QDialogButtonBox, QFrame, QMenu, QSpinBox, QTabWidget,
    QProgressBar, QCheckBox, QCalendarWidget, QDateEdit
)
...
class HistoryScheduleDialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        self._retention_spin.setRange(1, 3650)  # 1 day to 10 years
...
class HistoryTab(QWidget):
    ...
    - Creating and managing history collection schedules
    - Viewing historical data snapshots
    ...
    def _create_schedules_list_panel(self) -> QGroupBox:
        """Create the schedules list panel."""
        group = QGroupBox("History Schedules")
        layout = QVBoxLayout(group)

        self._schedules_table = QTableWidget()
        self._schedules_table.setColumnCount(6)
        self._schedules_table.setHorizontalHeaderLabels([
            "Name", "Connection", "Query", "Frequency", "Last Run", "Active"
        ])
    ...
    def _create_schedule_details_panel(self) -> QGroupBox:
        """Create the schedule details panel."""
    ...
    def _create_snapshot_details_panel(self) -> QGroupBox:
        """Create the snapshot details panel."""
    ...
    async def _refresh_history(self) -> None:
        """Refresh historical data entries."""
        try:
            # This would load history entries - placeholder for now
            ...
            for schedule_id, entries in entries_by_schedule.items():
                ...
                for entry in entries:
                    entry_item = QTreeWidgetItem([
                        "",  # Schedule (empty for child)
                        entry.get('table_name', 'Unknown'),
                        entry.get('collected_at', '').strftime('%Y-%m-%d %H:%M') if entry.get('collected_at') else '',
    ...
    async def _create_schedule_async(self, schedule: HistorySchedule) -> None:
        ...
        try:
            ...
            await self._plugin.create_history_schedule(
    ...
    async def _run_schedule_async(self, schedule_id: str) -> None:
        """Run schedule asynchronously."""
        try:
            self.operation_started.emit("Running history schedule...")

            # This would use the plugin's execute method when implemented
            # result = await self._plugin.execute_history_schedule_now(schedule_id)
    ...
    def _delete_schedule(self) -> None:
        ...
        reply = QMessageBox.question(
            self,
            "Confirm Deletion",
            f"Are you sure you want to delete the schedule '{schedule.get('name', 'Unknown')}'?\n\n"
    ...
    async def _delete_schedule_async(self, schedule_id: str) -> None:
        ...
        try:
            ...
            # success = await self._plugin.delete_history_schedule(schedule_id)
    ...
    def _show_schedule_context_menu(self, position) -> None:
        """Show context menu for schedules table."""
