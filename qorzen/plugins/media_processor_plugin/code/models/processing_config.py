...
class BackgroundRemovalConfig(BaseModel):
    ...
    method: BackgroundRemovalMethod = Field(
        default=BackgroundRemovalMethod.ALPHA_MATTING,
        description="Method used for background removal"
    )

    # Chroma key settings
    chroma_color: Optional[str] = Field(
        default="#00FF00",
        description="Color key to remove (for chroma key method)"
    )
    chroma_tolerance: int = Field(
        default=30,
        description="Tolerance for color matching (0-255)",
        ge=0,
        le=255
    )

    # Alpha matting settings
    alpha_foreground_threshold: int = Field(
        default=240,
        description="Threshold for foreground in alpha matting (0-255)",
        ge=0,
        le=255
    )
    alpha_background_threshold: int = Field(
        default=10,
        description="Threshold for background in alpha matting (0-255)",
        ge=0,
        le=255
    )
    ...
    confidence_threshold: float = Field(
        default=0.5,
        description="Confidence threshold for ML prediction (0.0-1.0)",
        ge=0.0,
        le=1.0
    )

    # Smart selection settings
    smart_brush_size: int = Field(
        default=20,
        description="Brush size for smart selection",
        ge=1
    )
    smart_feather_amount: int = Field(
        default=2,
        description="Feather amount for smart selection edges",
        ge=0
    )

    # Manual mask path (if applicable)
    mask_path: Optional[str] = Field(
        default=None,
        description="Path to manual mask file"
    )

    # Post-processing settings
    edge_feather: int = Field(
        default=2,
        description="Edge feathering amount to apply after removal",
        ge=0
    )
    refine_edge: bool = Field(
        default=True,
        description="Whether to apply edge refinement"
    )
    denoise: bool = Field(
        default=False,
        description="Apply denoising to mask"
    )
...
class WatermarkConfig(BaseModel):
    ...
    outline_color: Optional[str] = Field(
        default=None,
        description="Outline color for text watermark"
    )
    outline_width: int = Field(
        default=0,
        description="Outline width for text watermark",
        ge=0
    )
    ...
    scale: float = Field(
        default=0.2,
        description="Scale of watermark relative to image size",
        gt=0.0,
        le=1.0
    )
    margin: int = Field(
        default=10,
        description="Margin from edge of image (pixels)",
        ge=0
    )
    ...
    # Custom position (x, y) as percentage of image width/height
    custom_position_x: Optional[float] = Field(
        default=None,
        description="Custom X position as percentage of image width",
        ge=0.0,
        le=1.0
    )
    custom_position_y: Optional[float] = Field(
        default=None,
        description="Custom Y position as percentage of image height",
        ge=0.0,
        le=1.0
    )
...
class OutputFormat(BaseModel):
    ...
    id: str = Field(
        ...
        description="Unique identifier for this format"
    )
    name: str = Field(
        description="User-friendly name for this format"
    )
    ...
    prefix: Optional[str] = Field(
        default=None,
        description="Prefix to add to filename"
    )
    ...
    subdir: Optional[str] = Field(
        default=None,
        description="Subdirectory to save files in"
    )
    ...
    resize_mode: ResizeMode = Field(
        ...
        description="Method to use for resizing"
    )
    ...
    # Cropping
    crop_enabled: bool = Field(
        default=False,
        description="Whether to apply cropping"
    )
    crop_left: Optional[int] = Field(
        default=None,
        description="Left crop position",
        ge=0
    )
    crop_top: Optional[int] = Field(
        default=None,
        description="Top crop position",
        ge=0
    )
    crop_right: Optional[int] = Field(
        default=None,
        description="Right crop position",
        ge=0
    )
    crop_bottom: Optional[int] = Field(
        default=None,
        description="Bottom crop position",
        ge=0
    )
    ...
    padding_enabled: bool = Field(
        default=False,
        description="Whether to apply padding"
    )
    ...
    padding_color: Optional[str] = Field(
        default=None,
        description="Padding color (uses background_color if None)"
    )
    ...
    sharpness: float = Field(
        default=1.0,
        description="Sharpness adjustment factor",
        ge=0.0,
        le=2.0
    )
    ...
    preserve_exif: bool = Field(
        default=True,
        description="Whether to preserve original EXIF data"
    )
    custom_exif: Dict[str, str] = Field(
        default_factory=dict,
        description="Custom EXIF data to add"
    )
...
class ProcessingConfig(BaseModel):
    ...
    create_subfolder_for_batch: bool = Field(
        default=True,
        description="Create a subfolder for batch output"
    )
    batch_subfolder_template: str = Field(
        default="batch_{timestamp}",
        description="Template for batch subfolder name"
    )
