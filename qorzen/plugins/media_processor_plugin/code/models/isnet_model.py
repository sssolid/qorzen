...
def conv_bn_relu(in_ch: int, out_ch: int, kernel_size: int = 3, stride: int = 1, padding: int = 1,
    ...
    Returns:
        nn.Sequential: Sequential module with the given components
    """
...
class ResidualConv(nn.Module):
    ...
    def __init__(self, in_ch: int, out_ch: int) -> None:
        """
        Initialize the ResidualConv block.
...
class ISNetEncoder(nn.Module):
    ...
    def __init__(self, in_ch: int, depth: int = 64) -> None:
        """
        Initialize the ISNet encoder.

        Args:
            in_ch: Input channels
            depth: Base channel depth
        """
...
class ISNetDIS(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> tuple:
        ...
        Returns:
            Tuple of output tensor and side outputs
        """
