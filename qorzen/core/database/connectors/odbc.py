...
class ODBCConnector(BaseDatabaseConnector):
    ...
    def __init__(self, config: Any, logger: Any, security_manager: Optional[Any] = None) -> None:
        ...
        try:
            ...
            self._logger.error(
                "pyodbc module is required for ODBC connections. "
    ...
    async def connect(self) -> None:
        ...
        async with self._connect_lock:
            ...
            try:
                ...
                # Try to use the database manager if available
                ...
                # Direct connection if database_manager is not available or failed
                pyodbc = self._pyodbc
    ...
    async def _detect_filemaker_with_database_manager(self) -> bool:
        """Detect if the database is FileMaker when using database_manager.

        Returns:
            bool: True if the database is FileMaker
        """
        try:
    ...
    async def _get_filemaker_tables_with_database_manager(self) -> List[Dict[str, Any]]:
        """Get FileMaker tables using database_manager.
    ...
    async def _get_tables_by_sampling(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            List[Dict[str, Any]]: List of detected tables
        """
        common_tables = [
    ...
    async def _get_tables_direct(self, schema: Optional[str] = None) -> List[Dict[str, Any]]:
        """Get tables directly using pyodbc.
    ...
    async def _get_table_columns_with_database_manager(
            self,
            table_name: str,
            schema: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Try direct query if information schema approach fails
    ...
    def _process_information_schema_columns(
        ...
        """Process columns data from INFORMATION_SCHEMA.

        Args:
            columns_data: The columns data
            table_name: The table name

        Returns:
            List[Dict[str, Any]]: Processed column metadata
        """
        columns = []
    ...
    async def _get_table_columns_direct(
        ...
        """Get table columns directly using pyodbc.
    ...
    def _validate_and_prepare_query(
        ...
        # Special handling for FileMaker
        if self._is_filemaker and " " not in query:
            ...
            # Convert simple table name to SELECT * query with LIMIT
