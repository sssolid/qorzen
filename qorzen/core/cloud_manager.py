...
This module provides classes for managing connections to various cloud 
storage providers including AWS S3, Azure Blob Storage, and GCP Storage.
"""
...
class LocalStorageService(BaseCloudService):
    ...
    async def upload_file(self, local_path: str, remote_path: str) -> bool:
        ...
        try:
            if self._file_manager:
                await self._file_manager.copy_file(
                ...
                await loop.run_in_executor(
                    None, lambda: shutil.copy2(local_path, dest_path)
                )
    ...
    async def download_file(self, remote_path: str, local_path: str) -> bool:
        ...
        try:
            if self._file_manager:
                ...
                loop = asyncio.get_running_loop()
                await loop.run_in_executor(
                    None, lambda: shutil.copy2(source_path, local_path)
                )
    ...
    async def delete_file(self, remote_path: str) -> bool:
        ...
        try:
            if self._file_manager:
                await self._file_manager.delete_file(
    ...
    async def list_files(self, remote_path: str = '') -> List[Dict[str, Any]]:
        ...
        try:
            if self._file_manager:
                files = await self._file_manager.list_files(
                    path=os.path.join(self._base_directory, remote_path),
                    directory_type='base',
                    recursive=True
                )
...
class AWSStorageService(BaseCloudService):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # Create boto3 client in thread pool
            loop = asyncio.get_running_loop()
            ...
            await loop.run_in_executor(
                None,
                lambda: self._s3_client.head_bucket(Bucket=self._bucket)
            )
    ...
    async def upload_file(self, local_path: str, remote_path: str) -> bool:
        ...
        try:
            ...
            # Use asyncio to run S3 operations in a thread pool
            loop = asyncio.get_running_loop()
            await loop.run_in_executor(
                None,
                lambda: self._s3_client.upload_file(local_path, self._bucket, s3_key)
            )
    ...
    async def download_file(self, remote_path: str, local_path: str) -> bool:
        ...
        try:
            ...
            await loop.run_in_executor(
                None,
                lambda: self._s3_client.download_file(self._bucket, s3_key, local_path)
            )
    ...
    async def delete_file(self, remote_path: str) -> bool:
        ...
        try:
            ...
            await loop.run_in_executor(
                None,
                lambda: self._s3_client.delete_object(Bucket=self._bucket, Key=s3_key)
            )
    ...
    async def list_files(self, remote_path: str = '') -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Helper function to process paginator
    ...
    def _get_relative_path(self, s3_key: str) -> str:
        """
        Convert an S3 key to a relative path without prefix.
...
class AzureBlobStorageService(BaseCloudService):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # Create Azure clients in thread pool
            loop = asyncio.get_running_loop()

            if connection_string:
                self._blob_service_client = await loop.run_in_executor(
                    None,
                    lambda: BlobServiceClient.from_connection_string(connection_string)
                )
            else:
                from azure.storage.blob import BlobServiceClient
                ...
                self._blob_service_client = await loop.run_in_executor(
                    None,
                    lambda: BlobServiceClient(account_url=account_url, credential=account_key)
                )

            self._container_client = await loop.run_in_executor(
                None,
                lambda: self._blob_service_client.get_container_client(self._container)
            )

            # Check if container exists, create if not
            container_exists = await loop.run_in_executor(
                None,
                lambda: self._container_client.exists()
            )

            if not container_exists:
                await loop.run_in_executor(
                    None,
                    lambda: self._container_client.create_container()
                )
    ...
    async def shutdown(self) -> None:
        ...
        if self._blob_service_client:
            ...
            await loop.run_in_executor(
                None,
                lambda: self._blob_service_client.close()
            )
    ...
    async def upload_file(self, local_path: str, remote_path: str) -> bool:
        ...
        try:
            ...
            # Get blob client
            blob_client = await loop.run_in_executor(
                None,
                lambda: self._container_client.get_blob_client(blob_name)
            )
    ...
    def _get_blob_name(self, path: str) -> str:
        """
        Convert a relative path to a blob name with prefix.
    ...
    def _get_relative_path(self, blob_name: str) -> str:
        """
        Convert a blob name to a relative path without prefix.
...
class GCPStorageService(BaseCloudService):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # Create GCP clients in thread pool
            loop = asyncio.get_running_loop()

            self._storage_client = await loop.run_in_executor(
                None,
                lambda: storage.Client()
            )

            self._bucket_client = await loop.run_in_executor(
                None,
                lambda: self._storage_client.bucket(self._bucket)
            )

            # Check if bucket exists
            bucket_exists = await loop.run_in_executor(
                None,
                lambda: self._bucket_client.exists()
            )
    ...
    async def upload_file(self, local_path: str, remote_path: str) -> bool:
        ...
        try:
            ...
            blob = await loop.run_in_executor(
                None,
                lambda: self._bucket_client.blob(blob_name)
            )
...
class CloudManager(QorzenManager):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self._healthy = True
            self._logger.info(
                f'Cloud Manager initialized with provider: {self._provider.value}, storage backend: {self._storage_backend.value}')
        except Exception as e:
    ...
    async def _initialize_storage_service(self, config: Dict[str, Any]) -> None:
        """
        Initialize the storage service based on configured backend.
