...
class ApplicationCore:
        ...
        def wrap(step_fn, name):
            def wrapped():
                ...
                if progress_callback:
                    progress_callback(f'Initializing {name} ({current_index}/{total_steps})', int(current_index / total_steps * 100))
                step_fn()
                index_ref['index'] += 1
            return wrapped
    ...
    def finalize_initialization(self):
        ...
        if event_bus_manager:
            event_bus_manager.publish(event_type=EventType.SYSTEM_STARTED, source='app_core', payload={'version': self._get_version()})
    ...
    def _init_plugin_manager(self):
        ...
        plugin_manager = PluginManager(application_core=self, config_manager=config, logger_manager=logging, event_bus_manager=event_bus, file_manager=file, thread_manager=thread, database_manager=database, remote_service_manager=remote, security_manager=security, api_manager=api, cloud_manager=cloud)
    ...
    def _initialize_plugin_repository(self, config_manager: ConfigManager) -> PluginRepositoryManager:
        ...
        repository_manager = PluginRepositoryManager(config_file=repository_config_path if repository_config_path.exists() else None, logger=lambda msg, level: self._logger.log(level.upper(), msg) if self._logger else None)
    ...
    def _initialize_plugin_installer(self, config_manager: ConfigManager, repository_manager: PluginRepositoryManager, verifier: PluginVerifier) -> IntegratedPluginInstaller:
        plugins_dir = Path(config_manager.get('plugins.directory', 'plugins'))
        installer = IntegratedPluginInstaller(plugins_dir=plugins_dir, repository_manager=repository_manager, verifier=verifier, logger=lambda msg, level: self._logger.log(level.upper(), msg) if self._logger else None, core_version=self._get_version())
        return installer
    ...
    def set_main_window(self, main_window: Any) -> None:
        ...
        if event_bus and self._initialized:
            event_bus.publish(event_type=EventType.UI_READY, source='app_core', payload={'main_window': main_window})
            if self._logger:
    ...
    def status(self) -> Dict[str, Any]:
        status = {'name': 'ApplicationCore', 'initialized': self._initialized, 'managers': {}, 'version': self._get_version()}
