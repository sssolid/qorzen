...
class APIManager(QorzenManager):
    def __init__(self, config_manager: Any, logger_manager: Any, security_manager: Any, event_bus_manager: Any, concurrency_manager: Any, registry: Optional[Dict[str, Any]]=None) -> None:
    ...
    async def initialize(self) -> None:
        try:
            ...
            self._app = FastAPI(title='Qorzen API', description='API for the Qorzen platform', version='0.1.0', docs_url='/api/docs', redoc_url='/api/redoc', openapi_url='/api/openapi.json')
            ...
            await self._event_bus_manager.publish(event_type='api/started', source='api_manager', payload={'host': self._host, 'port': self._port, 'url': f'http://{self._host}:{self._port}/api'})
    ...
    async def _create_auth_router(self) -> APIRouter:
        ...
        async def login(form_data: OAuth2PasswordRequestForm=Depends()) -> Dict[str, Any]:
            user_data = await self._security_manager.authenticate_user(form_data.username, form_data.password)
            if not user_data:
    ...
    async def _create_users_router(self) -> APIRouter:
        ...
        async def create_user(user: UserCreate, current_user: Dict[str, Any]=Depends(self._get_current_admin_user)) -> Dict[str, Any]:
            try:
                user_id = await self._security_manager.create_user(username=user.username, email=user.email, password=user.password, roles=[await self._get_user_role(role) for role in user.roles], metadata=user.metadata)
                if not user_id:
        ...
        async def update_user(user_id: str, user_update: UserUpdate, current_user: Dict[str, Any]=Depends(self._get_current_admin_user)) -> Dict[str, Any]:
            ...
            if 'roles' in update_data:
                update_data['roles'] = [await self._get_user_role(role) for role in update_data['roles']]
            try:
    ...
    async def _create_system_router(self) -> APIRouter:
        ...
        async def get_system_status(current_user: Dict[str, Any]=Depends(self._get_current_user_with_permission('system.view'))) -> Dict[str, Any]:
            ...
            return {'name': 'Qorzen', 'initialized': True, 'healthy': True, 'managers': {}, 'version': '0.1.0'}
    ...
    async def _create_monitoring_router(self) -> APIRouter:
        ...
        async def get_alerts(include_resolved: bool=False, level: Optional[str]=None, metric_name: Optional[str]=None, current_user: Dict[str, Any]=Depends(self._get_current_user_with_permission('system.view'))) -> List[Dict[str, Any]]:
            ...
            return await self._registry['monitoring'].get_alerts(include_resolved=include_resolved, level=alert_level, metric_name=metric_name)
    ...
    async def _run_server(self) -> None:
        if not self._app:
            return
        config = uvicorn.Config(app=self._app, host=self._host, port=self._port, log_level='info', loop='asyncio')
    ...
    async def _on_config_changed(self, key: str, value: Any) -> None:
        if key == 'api.enabled':
            ...
            self._logger.warning(f'Changing {key} requires restart to take effect', extra={key.split('.')[-1]: value})
    async def register_api_endpoint(self, path: str, method: str, endpoint: Callable, tags: Optional[List[str]]=None, response_model: Optional[Any]=None, dependencies: Optional[List[Any]]=None, summary: Optional[str]=None, description: Optional[str]=None) -> bool:
    ...
    async def status(self) -> Dict[str, Any]:
        ...
        if self._initialized:
            is_server_running = self._server_task is not None and (not self._server_task.done()) and (not self._shutdown_event.is_set())
