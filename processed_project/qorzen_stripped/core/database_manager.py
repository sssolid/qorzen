...
class DatabaseManager(QorzenManager):
    ...
    def execute(self, statement: Any, connection_name: Optional[str]=None) -> List[Dict[str, Any]]:
        connection = self._get_connection(connection_name)
        ...
        try:
            ...
            self._logger.error(f'Database error: {str(e)}')
            raise DatabaseError(f'Database error: {str(e)}') from e
    def execute_raw(self, sql: str, params: Optional[Dict[str, Any]]=None, connection_name: Optional[str]=None) -> List[Dict[str, Any]]:
        connection = self._get_connection(connection_name)
        if not connection.engine:
            raise DatabaseError(f'Engine not initialized for connection {connection.config.name}')
        try:
        ...
        if self._initialized:
            ...
            with self._connections_lock:
                for name, connection in self._connections.items():
                    ...
                    connection_statuses[name] = {'database': {'type': connection.config.db_type, 'connection_ok': connection_ok, 'async_supported': connection.async_engine is not None}, 'pool': pool_status, 'sessions': {'active': len(connection.active_sessions)}, 'queries': query_stats}
