...
class DatabaseConnectionConfig:
    def __init__(self, name: str, db_type: str, host: str, port: int, database: str, user: str, password: str, pool_size: int=5, max_overflow: int=10, pool_recycle: int=3600, echo: bool=False) -> None:
        self.name = name
...
class DatabaseManager(QorzenManager):
    ...
    async def initialize(self) -> None:
        try:
            ...
            raise DatabaseManagerInitializationError(f'Failed to initialize AsyncDatabaseManager: {str(e)}', manager_name=self.name) from e
    async def _init_connection(self, connection: DatabaseConnection) -> None:
        ...
        if config.db_type == 'sqlite':
            ...
            if config.db_type == 'postgresql':
                db_async_url = URL.create('postgresql+asyncpg', username=config.user, password=config.password, host=config.host, port=config.port, database=config.database)
            else:
                db_async_url = None
        connection.engine = create_engine(db_url, pool_size=config.pool_size, max_overflow=config.max_overflow, pool_recycle=config.pool_recycle, echo=config.echo)
        if db_async_url:
    ...
    async def execute(self, statement: Any, connection_name: Optional[str]=None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self._logger.error(f'Database error: {str(e)}')
            raise DatabaseError(f'Database error: {str(e)}') from e
    async def execute_raw(self, sql: str, params: Optional[Dict[str, Any]]=None, connection_name: Optional[str]=None) -> List[Dict[str, Any]]:
        connection = await self._get_connection(connection_name)
        if not connection.engine:
            raise DatabaseError(f'Engine not initialized for connection {connection.config.name}')
        try:
    ...
    async def shutdown(self) -> None:
        ...
        try:
            ...
            raise ManagerShutdownError(f'Failed to shut down AsyncDatabaseManager: {str(e)}', manager_name=self.name) from e
        ...
        if self._initialized:
            ...
            for name, connection in self._connections.items():
                ...
                try:
                    ...
                    pass
                connection_statuses[name] = {'database': {'type': connection.config.db_type, 'connection_ok': connection_ok, 'async_supported': connection.async_engine is not None}, 'pool': pool_status, 'sessions': {'active_sync': len(connection.active_sessions), 'active_async': len(connection.active_async_sessions)}, 'queries': query_stats}
            status.update({'connections': connection_statuses, 'default_connection': 'default'})
        return status