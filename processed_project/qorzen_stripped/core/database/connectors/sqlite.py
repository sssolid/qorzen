...
class AsyncCompatibleSession:
    ...
    async def execute(self, statement, *args, **kwargs) -> Any:
        return await self._loop.run_in_executor(None, lambda: self._session.execute(statement, *args, **kwargs))
...
class SQLiteConnector(BaseDatabaseConnector):
    ...
    def _create_database_manager_config(self) -> Any:
        ...
        return DatabaseConnectionConfig(name=self._registered_connection_id or f'sqlite_{self._config.name}', db_type='sqlite', host='', port=0, database=db_path, user='', password='', pool_size=1, max_overflow=0, pool_recycle=3600, echo=False, read_only=self._config.read_only, allowed_tables=self._config.allowed_tables)
    async def connect(self) -> None:
        async with self._connect_lock:
            ...
            try:
                db_path = self._config.database
                self._logger.info('Connecting to SQLite database', extra={'database': db_path, 'read_only': self._config.read_only})
                start_time = time.time()
                if self._database_manager:
                    success = await self._register_with_database_manager()
                    if success:
                        try:
                            await self._database_manager.execute_raw(sql='SELECT 1', connection_name=self._registered_connection_id)
                            self._connection_time = time.time() - start_time
                            self._connected = True
                            self._logger.info('Successfully connected to SQLite database via database_manager', extra={'database': db_path, 'connection_time_ms': int(self._connection_time * 1000), 'sqlite_version': self._sqlite_version})
                            return
                ...
                try:
                    ...
                    if db_path == ':memory:':
                        ...
                        if self._config.read_only:
                            uri_path += '?mode=ro'
                    self._engine = create_engine(uri_path, connect_args={'timeout': getattr(self._config, 'connection_timeout', 10)}, echo=False)
                    ...
                    self._connection_time = time.time() - start_time
                    self._connected = True
                    self._logger.info('Successfully connected to SQLite database directly', extra={'database': db_path, 'connection_time_ms': int(self._connection_time * 1000), 'sqlite_version': self._sqlite_version})
                except ImportError:
                    import aiosqlite
                    if db_path == ':memory:':
                        ...
                        if self._config.read_only:
                            uri_path += '?mode=ro'
                        self._connection = await aiosqlite.connect(uri_path, uri=True, timeout=getattr(self._config, 'connection_timeout', 10))
                    await self._connection.execute('PRAGMA foreign_keys = ON')
                    self._connection.row_factory = self._dict_factory
                    self._connection_time = time.time() - start_time
                    self._connected = True
                    self._logger.info('Successfully connected to SQLite database using aiosqlite', extra={'database': db_path, 'connection_time_ms': int(self._connection_time * 1000), 'sqlite_version': self._sqlite_version})
            except Exception as e:
                error_msg = str(e)
                ...
                if 'readonly' in error_msg.lower() or 'permission' in error_msg.lower():
                    raise SecurityError(message=f'Security error connecting to SQLite: {sanitized_error}', details={'original_error': sanitized_error}) from e
                else:
                    raise DatabaseError(message=f'Failed to connect to SQLite database: {sanitized_error}', details={'original_error': sanitized_error}) from e
    async def disconnect(self) -> None:
        ...
        try:
            ...
            raise DatabaseError(message=f'Failed to close SQLite connection: {str(e)}', details={'original_error': str(e)}) from e
    async def execute_query(self, query: str, params: Optional[Dict[str, Any]]=None, limit: Optional[int]=None) -> Dict[str, Any]:
        ...
        try:
            ...
            self._logger.debug('Executing SQLite query', extra={'query': sanitized_query, 'limit': limit, 'using_db_manager': self._registered_connection_id is not None})
            if self._registered_connection_id and self._database_manager:
                ...
                self._logger.info('Successfully executed query on SQLite via database_manager', extra={'record_count': result['row_count'], 'execution_time_ms': result['execution_time_ms'], 'table': table_name if table_name else None})
                return result
            start_time = time.time()
            if self._engine and self._session_factory:
                ...
                columns = [{'name': name, 'type_name': 'UNKNOWN', 'type_code': 0, 'precision': 0, 'scale': 0, 'nullable': True, 'table_name': None} for name in column_names]
            elif self._connection:
                if params:
                ...
                raise DatabaseError(message='No database connection available', details={'query': sanitized_query})
            ...
            self._logger.info('Successfully executed query on SQLite directly', extra={'record_count': result['row_count'], 'execution_time_ms': result['execution_time_ms'], 'table': table_name if table_name else None})
            return result
        except Exception as e:
            error_msg = str(e)
            sanitized_error = self._sanitize_error_message(error_msg)
            self._logger.error('Error executing query on SQLite', extra={'error': sanitized_error, 'query': self._sanitize_sql_for_logging(query)})
            ...
            if any((keyword in error_msg.lower() for keyword in ['permission', 'access denied', 'authorization'])):
                raise SecurityError(message=f'Security error executing SQLite query: {sanitized_error}', details={'original_error': sanitized_error, 'query': self._sanitize_sql_for_logging(query)}) from e
            else:
                raise DatabaseError(message=f'Failed to execute SQLite query: {sanitized_error}', details={'original_error': sanitized_error, 'query': self._sanitize_sql_for_logging(query)}) from e
        finally:
    ...
    async def get_tables(self, schema: Optional[str]=None) -> List[Dict[str, Any]]:
        ...
        try:
            query = "\n                    SELECT name, type\n                    FROM sqlite_master\n                    WHERE type = 'table'\n                       OR type = 'view'\n                    ORDER BY name                     "
            if self._registered_connection_id and self._database_manager:
                table_rows = await self._database_manager.execute_raw(sql=query, connection_name=self._registered_connection_id)
            ...
            for row in table_rows:
                ...
                tables.append({'name': table_name, 'type': table_type, 'schema': None, 'remarks': None, 'columns': columns})
            return tables
        except Exception as e:
            error_msg = str(e)
            sanitized_error = self._sanitize_error_message(error_msg)
    ...
    async def get_table_columns(self, table_name: str, schema: Optional[str]=None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if self._registered_connection_id and self._database_manager:
                column_rows = await self._database_manager.execute_raw(sql=query, connection_name=self._registered_connection_id)
            elif self._engine and self._session_factory:
                loop = asyncio.get_running_loop()
                def execute_sync():
                    with self._session_factory() as session:
            ...
            for row in column_rows:
                ...
                columns.append({'name': col_name, 'type_name': col_type, 'type_code': 99, 'precision': precision, 'scale': scale, 'nullable': nullable, 'table_name': table_name, 'remarks': None})
            return columns
        except Exception as e:
            error_msg = str(e)
            sanitized_error = self._sanitize_error_message(error_msg)
            self._logger.error(f'Error getting column information: {sanitized_error}')
            raise DatabaseError(message=f'Failed to get column information: {sanitized_error}', details={'table': table_name}) from e
    def get_connection_info(self) -> Dict[str, Any]:
        info = {'connected': self._connected, 'connection_id': self._config.name, 'name': self._config.name, 'database': self._config.database, 'type': 'SQLite', 'read_only': self._config.read_only, 'version': getattr(self, '_sqlite_version', 'Unknown'), 'using_db_manager': self._registered_connection_id is not None, 'using_sqlalchemy': self._engine is not None}
    ...
    async def async_session(self) -> AsyncGenerator[AsyncCompatibleSession, None]:
        ...
        if not self._engine or not self._session_factory:
            raise DatabaseError(message='SQLite engine or session factory not initialized', details={'connection_name': self._config.name})
        loop = asyncio.get_running_loop()
        ...
        try:
            ...
            raise DatabaseError(message=f'Error during SQLite async session: {str(e)}', details={'original_error': str(e)}) from e
        finally:
            await loop.run_in_executor(None, session.close)
    ...
    async def _get_column_metadata(self, cursor: Any) -> List[Dict[str, Any]]:
        ...
        if cursor.description:
            for i, col_desc in enumerate(cursor.description):
                name = col_desc[0]
                columns.append({'name': name, 'type_name': 'UNKNOWN', 'type_code': 0, 'precision': 0, 'scale': 0, 'nullable': True, 'table_name': None})
        return columns
