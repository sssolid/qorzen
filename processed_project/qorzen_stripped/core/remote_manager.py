...
class RemoteServicesManager(QorzenManager):
    def __init__(self, config_manager: Any, logger_manager: Any, event_bus_manager: Any, task_manager: Any) -> None:
    ...
    async def initialize(self) -> None:
        try:
            ...
            await self._event_bus_manager.subscribe(event_type='remote_service/register', callback=self._on_service_register_event, subscriber_id='remote_manager')
            await self._event_bus_manager.subscribe(event_type='remote_service/unregister', callback=self._on_service_unregister_event, subscriber_id='remote_manager')
            await self._config_manager.register_listener('remote_services', self._on_config_changed)
    ...
    async def _register_service_from_config(self, service_name: str, service_config: Dict[str, Any]) -> None:
    ...
    async def register_service(self, service: RemoteService) -> None:
        ...
        await self._event_bus_manager.publish(event_type='remote_service/registered', source='remote_manager', payload={'service_name': service.name, 'protocol': service.protocol.value, 'base_url': service.base_url})
    async def unregister_service(self, service_name: str) -> bool:
        ...
        await self._event_bus_manager.publish(event_type='remote_service/unregistered', source='remote_manager', payload={'service_name': service_name})
        return True
    ...
    async def _health_check_task(self) -> None:
        ...
        try:
            ...
            await self._event_bus_manager.publish(event_type='remote_service/health_check', source='remote_manager', payload={'services': {name: {'healthy': status} for name, status in health_statuses.items()}, 'healthy_count': healthy_count, 'unhealthy_count': unhealthy_count, 'timestamp': time.time()})
        except Exception as e:
    ...
    async def _schedule_health_checks(self) -> None:
        ...
        self._health_check_task_id = await self._task_manager.submit_periodic_task(self._health_check_task, interval=self._health_check_interval, name='service_health_check')
    ...
    async def make_request(self, service_name: str, method: str, path: str, **kwargs: Any) -> Any:
        if not self._initialized:
            raise ValueError('Remote Services Manager not initialized')
