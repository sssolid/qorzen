...
class PluginManifest:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> PluginManifest:
        return cls(name=data.get('name', ''), display_name=data.get('display_name', ''), version=data.get('version', '0.1.0'), description=data.get('description', ''), author=data.get('author', 'Unknown'), logo_path=data.get('logo_path', ''), icon_path=data.get('icon_path', ''), homepage=data.get('homepage'), license=data.get('license'), min_core_version=data.get('min_core_version'), max_core_version=data.get('max_core_version'), entry_point=data.get('entry_point', 'plugin.py'), dependencies=data.get('dependencies', []), capabilities=data.get('capabilities', []), resources=data.get('resources', {}), ui_integration=data.get('ui_integration', True), settings_schema=data.get('settings_schema'), metadata=data.get('metadata', {}))
    @classmethod
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'name': self.name, 'display_name': self.display_name, 'version': self.version, 'description': self.description, 'author': self.author, 'homepage': self.homepage, 'license': self.license, 'min_core_version': self.min_core_version, 'max_core_version': self.max_core_version, 'entry_point': self.entry_point, 'dependencies': self.dependencies, 'capabilities': self.capabilities, 'resources': self.resources, 'ui_integration': self.ui_integration, 'settings_schema': self.settings_schema, 'metadata': self.metadata}
@dataclass
class PluginInfo:
    plugin_id: str
    name: str
...
class PluginManager(QorzenManager):
    def __init__(self, application_core: Any, config_manager: Any, logger_manager: Any, event_bus_manager: Any, file_manager: Any, task_manager: Any, plugin_isolation_manager: Any) -> None:
    ...
    async def initialize(self) -> None:
        try:
            ...
            await self._event_bus_manager.subscribe(event_type='plugin/installed', callback=self._on_plugin_installed, subscriber_id='plugin_manager')
            await self._event_bus_manager.subscribe(event_type='plugin/uninstalled', callback=self._on_plugin_uninstalled, subscriber_id='plugin_manager')
            await self._event_bus_manager.subscribe(event_type='plugin/enabled', callback=self._on_plugin_enabled, subscriber_id='plugin_manager')
            await self._event_bus_manager.subscribe(event_type='plugin/disabled', callback=self._on_plugin_disabled, subscriber_id='plugin_manager')
            await self._event_bus_manager.subscribe(event_type='ui/ready', callback=self._on_ui_ready, subscriber_id='plugin_manager')
            ...
            await self._event_bus_manager.publish(event_type='plugin_manager/initialized', source='plugin_manager', payload={'plugin_count': len(self._plugins)})
        except Exception as e:
        ...
        try:
            ...
            for entry_point in entry_points:
                ...
                try:
                    ...
                    plugin_info = await self._extract_plugin_metadata(plugin_class=plugin_class, default_name=entry_point.name, plugin_id=plugin_id, metadata={'entry_point': entry_point.name})
                    async with self._plugins_lock:
                        self._plugins[plugin_id] = plugin_info
                    self._logger.debug(f"Discovered plugin '{plugin_info.name}' from entry point", extra={'plugin_id': plugin_id, 'version': plugin_info.version})
                except Exception as e:
    ...
    async def _discover_plugin_from_manifest(self, plugin_dir: pathlib.Path) -> None:
        ...
        try:
            ...
            self._logger.debug(f'Discovered plugin from manifest: {manifest.name}, entry point: {entry_point}, exists: {entry_exists}')
            if not entry_path.exists() and '.' in entry_point:
                module_name = f'{plugin_dir.name}.{entry_point}'
                try:
                    importlib.import_module(module_name)
                except ImportError:
                    self._logger.warning(f'Failed to import module {module_name} for plugin {manifest.name}', extra={'plugin': manifest.name, 'module': module_name})
                    return
            plugin_info = PluginInfo(plugin_id=plugin_id, name=manifest.name, version=manifest.version, description=manifest.description, author=manifest.author, state=PluginState.DISCOVERED, path=str(plugin_dir), manifest=manifest, dependencies=manifest.dependencies, capabilities=manifest.capabilities, metadata={'manifest': True, 'display_name': manifest.display_name, 'license': manifest.license, 'homepage': manifest.homepage, 'entry_point': manifest.entry_point, 'ui_integration': manifest.ui_integration})
            async with self._plugins_lock:
                self._plugins[plugin_id] = plugin_info
                self._plugin_paths[plugin_id] = str(plugin_dir)
            self._logger.debug(f"Discovered plugin '{plugin_info.name}' from manifest", extra={'plugin_id': plugin_id, 'version': plugin_info.version, 'path': str(plugin_dir)})
        except Exception as e:
            self._logger.error(f'Failed to discover plugin from manifest in {plugin_dir.name}: {str(e)}', extra={'directory': str(plugin_dir)}, exc_info=True)
        ...
        try:
            ...
            if hasattr(module, '__all__'):
                for name in module.__all__:
                    ...
                    if inspect.isclass(obj) and hasattr(obj, 'name') and hasattr(obj, 'version') and hasattr(obj, 'description'):
                        plugin_class = obj
                        break
            if not plugin_class:
            ...
            plugin_info = await self._extract_plugin_metadata(plugin_class=plugin_class, default_name=module_name, plugin_id=plugin_id, path=str(plugin_dir))
            async with self._plugins_lock:
                self._plugins[plugin_id] = plugin_info
                self._plugin_paths[plugin_id] = str(plugin_dir)
            self._logger.debug(f"Discovered plugin '{plugin_info.name}' from module", extra={'plugin_id': plugin_id, 'version': plugin_info.version, 'path': str(plugin_dir)})
        except Exception as e:
            self._logger.error(f'Failed to discover plugin from module in {plugin_dir.name}: {str(e)}', extra={'directory': str(plugin_dir)})
    async def _discover_installed_plugins(self) -> None:
        pass
    async def _extract_plugin_metadata(self, plugin_class: Type, default_name: str, plugin_id: str, path: Optional[str]=None, metadata: Optional[Dict[str, Any]]=None) -> PluginInfo:
        ...
        plugin_metadata = {'class': plugin_class.__name__, 'module': plugin_class.__module__}
        if metadata:
            plugin_metadata.update(metadata)
        plugin_info = PluginInfo(plugin_id=plugin_id, name=name, version=version, description=description, author=author, state=PluginState.DISCOVERED, path=path, dependencies=dependencies, capabilities=capabilities, metadata=plugin_metadata)
        return plugin_info
    async def _load_enabled_plugins(self) -> None:
        ...
        for plugin_id, _ in sorted_plugins:
            try:
                ...
                self._logger.error(f'Failed to load plugin {plugin_id}: {str(e)}', extra={'plugin_id': plugin_id, 'error': str(e)})
    async def _sort_plugins_by_dependencies(self, plugins: List[Tuple[str, PluginInfo]]) -> List[Tuple[str, PluginInfo]]:
        graph = {}
    ...
    async def load_plugin(self, plugin_id: str) -> bool:
        ...
        error_boundary = create_error_boundary(source='plugin_loading', plugin_id=plugin_id) if self._error_handler else None
        try:
            ...
            await self._event_bus_manager.publish(event_type='plugin/loaded', source='plugin_manager', payload={'plugin_id': plugin_id, 'name': plugin_info.name, 'version': plugin_info.version, 'description': plugin_info.description, 'author': plugin_info.author})
            self._logger.info(f"Loaded plugin '{plugin_info.name}' v{plugin_info.version}", extra={'plugin_id': plugin_id})
            return True
        except Exception as e:
            self._logger.error(f"Failed to load plugin '{plugin_id}': {str(e)}", extra={'plugin_id': plugin_id, 'error': str(e)}, exc_info=True)
            async with self._plugins_lock:
                plugin_info.state = PluginState.FAILED
                plugin_info.error = str(e)
            await self._event_bus_manager.publish(event_type='plugin/error', source='plugin_manager', payload={'plugin_id': plugin_id, 'error': str(e), 'traceback': traceback.format_exc()})
    ...
    async def _initialize_plugin(self, plugin_instance: Any, plugin_id: str) -> None:
        try:
            ...
            error_boundary = create_error_boundary(source='plugin_initialization', plugin_id=plugin_id) if self._error_handler else None
            if error_boundary:
                if asyncio.iscoroutinefunction(initialize_method):
                    await error_boundary.run(initialize_method, self._application_core, severity=ErrorSeverity.HIGH)
                else:
                    await error_boundary.run(lambda: initialize_method(self._application_core), severity=ErrorSeverity.HIGH)
    ...
    async def _setup_plugin_ui(self, plugin_id: str) -> None:
        ...
        async with self._plugins_lock:
            ...
            if not hasattr(plugin_instance, 'setup_ui'):
                return
        error_boundary = create_error_boundary(source='plugin_ui_setup', plugin_id=plugin_id) if self._error_handler else None
        try:
            setup_ui_method = getattr(plugin_instance, 'setup_ui')
            if error_boundary:
                if asyncio.iscoroutinefunction(setup_ui_method):
                    await error_boundary.run(setup_ui_method, self._ui_integration, severity=ErrorSeverity.MEDIUM)
                else:
                    await error_boundary.run(lambda: setup_ui_method(self._ui_integration), severity=ErrorSeverity.MEDIUM)
            ...
            self._logger.error(f"Failed to set up UI for plugin '{plugin_id}': {str(e)}", extra={'plugin_id': plugin_id, 'error': str(e)}, exc_info=True)
            ...
            for other_id, other_info in self._plugins.items():
                if other_id != plugin_id and other_info.state == PluginState.ACTIVE:
                    if plugin_info.name in other_info.dependencies:
                        raise PluginError(f"Cannot unload plugin '{plugin_id}' because it's a dependency for '{other_id}'")
        error_boundary = create_error_boundary(source='plugin_unloading', plugin_id=plugin_id) if self._error_handler else None
        try:
            ...
            await self._event_bus_manager.publish(event_type='plugin/unloaded', source='plugin_manager', payload={'plugin_id': plugin_id, 'name': plugin_info.name})
            self._logger.info(f"Unloaded plugin '{plugin_info.name}'", extra={'plugin_id': plugin_id})
            return True
        except Exception as e:
            self._logger.error(f"Failed to unload plugin '{plugin_id}': {str(e)}", extra={'plugin_id': plugin_id, 'error': str(e)}, exc_info=True)
            await self._event_bus_manager.publish(event_type='plugin/error', source='plugin_manager', payload={'plugin_id': plugin_id, 'error': f'Failed to unload plugin: {str(e)}', 'traceback': traceback.format_exc()})
    ...
    async def _cleanup_plugin_ui(self, plugin_id: str) -> None:
        ...
        try:
            ...
            self._logger.error(f"Error cleaning up UI for plugin '{plugin_id}': {str(e)}", extra={'plugin_id': plugin_id, 'error': str(e)})
    ...
    async def reload_plugin(self, plugin_id: str) -> bool:
        ...
        try:
            ...
            self._logger.error(f"Failed to reload plugin '{plugin_id}': {str(e)}", extra={'plugin_id': plugin_id, 'error': str(e)}, exc_info=True)
            await self._event_bus_manager.publish(event_type='plugin/error', source='plugin_manager', payload={'plugin_id': plugin_id, 'error': f'Failed to reload plugin: {str(e)}', 'traceback': traceback.format_exc()})
    ...
    async def _on_plugin_installed(self, event: Any) -> None:
        ...
        if auto_enable:
            try:
                ...
                self._logger.error(f"Failed to enable installed plugin '{plugin_id}': {str(e)}", extra={'plugin_id': plugin_id, 'error': str(e)})
    ...
    async def _on_plugin_enabled(self, event: Any) -> None:
        ...
        try:
            ...
            self._logger.error(f"Failed to handle plugin enabled event for '{plugin_id}': {str(e)}", extra={'plugin_id': plugin_id, 'error': str(e)})
    async def _on_plugin_disabled(self, event: Any) -> None:
        ...
        try:
            ...
            self._logger.error(f"Failed to handle plugin disabled event for '{plugin_id}': {str(e)}", extra={'plugin_id': plugin_id, 'error': str(e)})
    async def get_plugin_info(self, plugin_id: str) -> Optional[PluginInfo]:
        async with self._plugins_lock:
            return self._plugins.get(plugin_id)
    async def get_plugins(self, state: Optional[Union[PluginState, List[PluginState]]]=None) -> Dict[str, PluginInfo]:
        states = None
        ...
        async with self._plugins_lock:
            if states:
                return {pid: pinfo for pid, pinfo in self._plugins.items() if pinfo.state in states}
            else:
        ...
        try:
            ...
            for plugin_id in reversed(loaded_plugins):
                try:
                    ...
                    self._logger.error(f"Error unloading plugin '{plugin_id}' during shutdown: {str(e)}", extra={'plugin_id': plugin_id, 'error': str(e)})
    ...
    def status(self) -> Dict[str, Any]:
        ...
        if self._initialized:
            ...
            status.update({'plugins': {'total': len(self._plugins), 'by_state': state_counts, 'loaded': len(self._plugin_instances), 'ui_integration': len(self._ui_plugins)}, 'config': {'auto_load': self._auto_load, 'plugin_dir': str(self._plugin_dir) if self._plugin_dir else None, 'enabled_count': len(self._enabled_plugins), 'disabled_count': len(self._disabled_plugins)}, 'ui_ready': self._ui_ready.is_set()})
        return status