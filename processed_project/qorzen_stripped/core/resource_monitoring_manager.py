...
class ResourceMonitoringManager(QorzenManager):
    ...
    async def initialize(self) -> None:
        try:
            ...
            self._alert_thresholds = {'cpu_percent': alert_config.get('cpu_percent', 80.0), 'memory_percent': alert_config.get('memory_percent', 80.0), 'disk_percent': alert_config.get('disk_percent', 90.0)}
            ...
            await self._event_bus_manager.subscribe(event_type='*', callback=self._on_event, subscriber_id='monitoring_manager')
            await self._config_manager.register_listener('monitoring', self._on_config_changed)
            await self._schedule_metric_collection()
            await self._event_bus_manager.publish(event_type='monitoring/initialized', source='monitoring_manager', payload={'prometheus_port': self._prometheus_server_port})
            self._initialized = True
            self._healthy = True
            ...
            raise ManagerInitializationError(f'Failed to initialize AsyncResourceMonitoringManager: {str(e)}', manager_name=self.name) from e
    async def _schedule_metric_collection(self) -> None:
        for task in self._collection_tasks.values():
            task.cancel()
        self._collection_tasks.clear()
        system_metrics_task = asyncio.create_task(self._collect_system_metrics_loop(), name='system_metrics_collection')
        self._collection_tasks['system_metrics'] = system_metrics_task
        uptime_task = asyncio.create_task(self._collect_uptime_metrics_loop(), name='uptime_metrics_collection')
        ...
        try:
            ...
            if self._cpu_percent_gauge:
                if self._thread_manager and (not self._thread_manager.is_main_thread()):
                    await self._thread_manager.run_on_main_thread(lambda: self._cpu_percent_gauge.set(cpu_percent))
                else:
            ...
            if self._memory_percent_gauge:
                if self._thread_manager and (not self._thread_manager.is_main_thread()):
                    await self._thread_manager.run_on_main_thread(lambda: self._memory_percent_gauge.set(memory_percent))
                else:
            ...
            if self._disk_percent_gauge:
                if self._thread_manager and (not self._thread_manager.is_main_thread()):
                    await self._thread_manager.run_on_main_thread(lambda: self._disk_percent_gauge.set(disk_percent))
                else:
            ...
            await self._event_bus_manager.publish(event_type='monitoring/metrics', source='monitoring_manager', payload={'cpu_percent': cpu_percent, 'memory_percent': memory_percent, 'disk_percent': disk_percent, 'timestamp': time.time()})
        except Exception as e:
    ...
    async def _collect_uptime_metrics(self) -> None:
        try:
            ...
            if 'app_uptime_seconds' in self._metrics:
                if self._thread_manager and (not self._thread_manager.is_main_thread()):
                    await self._thread_manager.run_on_main_thread(lambda: self._metrics['app_uptime_seconds'].set(uptime_seconds))
                else:
    ...
    async def _check_threshold(self, metric_name: str, value: float) -> None:
        ...
        if value >= threshold * 1.25:
            await self._create_alert(level=AlertLevel.CRITICAL, message=f"{metric_name.replace('_', ' ').title()} is critically high: {value:.1f}%", source='monitoring_manager', metric_name=metric_name, metric_value=value, threshold=threshold)
        elif value >= threshold:
            await self._create_alert(level=AlertLevel.WARNING, message=f"{metric_name.replace('_', ' ').title()} is high: {value:.1f}%", source='monitoring_manager', metric_name=metric_name, metric_value=value, threshold=threshold)
        else:
            await self._resolve_alerts_for_metric(metric_name)
    async def _create_alert(self, level: AlertLevel, message: str, source: str, metric_name: Optional[str]=None, metric_value: Optional[float]=None, threshold: Optional[float]=None, metadata: Optional[Dict[str, Any]]=None) -> str:
        import uuid
        if metric_name:
        ...
        await self._event_bus_manager.publish(event_type='monitoring/alert', source='monitoring_manager', payload={'alert_id': alert_id, 'level': level.value, 'message': message, 'timestamp': alert.timestamp.isoformat(), 'metric_name': metric_name, 'metric_value': metric_value, 'threshold': threshold})
        return alert_id
        ...
        async with self._alerts_lock:
            for alert_id, alert in list(self._alerts.items()):
                if alert.metric_name == metric_name and (not alert.resolved):
                    ...
                    await self._event_bus_manager.publish(event_type='monitoring/alert_resolved', source='monitoring_manager', payload={'alert_id': alert_id, 'metric_name': metric_name, 'resolved_at': alert.resolved_at.isoformat()})
    async def _on_event(self, event: Any) -> None:
        if 'events_total' in self._metrics:
            if self._thread_manager and (not self._thread_manager.is_main_thread()):
                await self._thread_manager.run_on_main_thread(lambda: self._metrics['events_total'].labels(event_type=event.event_type, source=event.source).inc())
            else:
    ...
    async def _on_config_changed(self, key: str, value: Any) -> None:
        if key == 'monitoring.enabled':
            ...
            await self._schedule_metric_collection()
    async def register_gauge(self, name: str, description: str, labels: Optional[List[str]]=None) -> Any:
        ...
        return gauge
    async def register_counter(self, name: str, description: str, labels: Optional[List[str]]=None) -> Any:
        if not self._initialized:
            raise ValueError('AsyncResourceMonitoringManager is not initialized')
        if name in self._metrics:
            raise ValueError(f"Metric '{name}' is already registered")
        ...
        return counter
    async def register_histogram(self, name: str, description: str, labels: Optional[List[str]]=None, buckets: Optional[List[float]]=None) -> Any:
        if not self._initialized:
            raise ValueError('AsyncResourceMonitoringManager is not initialized')
        if name in self._metrics:
            raise ValueError(f"Metric '{name}' is already registered")
        ...
        return histogram
    async def register_summary(self, name: str, description: str, labels: Optional[List[str]]=None) -> Any:
        if not self._initialized:
            raise ValueError('AsyncResourceMonitoringManager is not initialized')
        if name in self._metrics:
            raise ValueError(f"Metric '{name}' is already registered")
        ...
        return summary
    async def get_alerts(self, include_resolved: bool=False, level: Optional[AlertLevel]=None, metric_name: Optional[str]=None) -> List[Dict[str, Any]]:
        result = []
        async with self._alerts_lock:
    ...
    async def shutdown(self) -> None:
        ...
        try:
            ...
            raise ManagerShutdownError(f'Failed to shut down AsyncResourceMonitoringManager: {str(e)}', manager_name=self.name) from e
