...
class EventBusManager(QorzenManager):
                    ...
                    for subscription in subscriptions:
                        try:
                            if needs_main_thread and self._thread_manager:
                                self._thread_manager.run_on_main_thread(lambda s=subscription, e=event: s.callback(e))
                            else:
    ...
    def publish(self, event_type: Union[EventType, str], source: str, payload: Optional[Dict[str, Any]]=None, correlation_id: Optional[str]=None, synchronous: bool=False) -> str:
    ...
    def _process_event_sync(self, event: Event, subscriptions: List[EventSubscription]) -> None:
        for subscription in subscriptions:
            try:
                ...
                self._logger.error(f'Error in event handler for {event.event_type}: {str(e)}', extra={'event_id': event.event_id, 'subscription_id': subscription.subscriber_id, 'error': str(e)})
    def subscribe(self, event_type: Union[EventType, str], callback: EventHandler, subscriber_id: Optional[str]=None, filter_criteria: Optional[Dict[str, Any]]=None) -> str:
        if not self._initialized:
