...
class ConnectionDialog(QDialog):
    ...
    def _test_connection(self) -> None:
        try:
            ...
            QMessageBox.information(self, 'Connection Test', 'Connection test would normally connect to the database.\n\nPlease click OK to proceed with the test.')
            import asyncio
    ...
    def get_connection_config(self) -> BaseConnectionConfig:
        ...
        common_params = {'id': self._connection.id if self._connection else str(uuid.uuid4()), 'name': self._name_edit.text().strip(), 'connection_type': connection_type, 'database': self._database_edit.text().strip(), 'username': self._username_edit.text().strip(), 'password': self._password_edit.text(), 'connection_timeout': self._conn_timeout_spin.value(), 'query_timeout': self._query_timeout_spin.value(), 'encrypt_connection': self._encrypt_checkbox.isChecked(), 'read_only': self._readonly_checkbox.isChecked()}
        ...
        if allowed_tables_text:
            common_params['allowed_tables'] = [t.strip().upper() for t in allowed_tables_text.split(',') if t.strip()]
        if connection_type == ConnectionType.AS400:
            as400_params = {'server': self._as400_server_edit.text().strip(), 'port': self._as400_port_spin.value(), 'ssl': self._as400_ssl_checkbox.isChecked(), 'jt400_jar_path': self._as400_jar_edit.text().strip()}
            ...
            if allowed_libs_text:
                as400_params['allowed_libraries'] = [lib.strip().upper() for lib in allowed_libs_text.split(',') if lib.strip()]
            return AS400ConnectionConfig(**common_params, **as400_params)
        elif connection_type == ConnectionType.ODBC:
            odbc_params = {'dsn': self._odbc_dsn_edit.text().strip(), 'server': self._odbc_server_edit.text().strip() or None, 'port': self._odbc_port_spin.value() if self._odbc_port_spin.value() > 0 else None, 'connection_string': self._odbc_connection_string_edit.text().strip() or None}
...
class ConnectionManagerDialog(QDialog):
    ...
    def _test_connection(self) -> None:
        ...
        if conn_id not in self._connections:
            return
        conn = self._connections[conn_id]
        QMessageBox.information(self, 'Connection Test', f'Testing connection to {conn.name}...\n\nPlease click OK to proceed with the test.')
        import asyncio
