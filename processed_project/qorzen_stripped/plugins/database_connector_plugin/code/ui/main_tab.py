...
class SQLHighlighter(QSyntaxHighlighter):
    def __init__(self, document: QTextDocument) -> None:
        ...
        keywords = ['SELECT', 'FROM', 'WHERE', 'JOIN', 'INNER', 'LEFT', 'RIGHT', 'FULL', 'ON', 'GROUP', 'BY', 'HAVING', 'ORDER', 'LIMIT', 'OFFSET', 'UNION', 'INSERT', 'UPDATE', 'DELETE', 'CREATE', 'ALTER', 'DROP', 'TABLE', 'INDEX', 'VIEW', 'PROCEDURE', 'FUNCTION', 'TRIGGER', 'DATABASE', 'SCHEMA', 'AND', 'OR', 'NOT', 'IN', 'EXISTS', 'BETWEEN', 'LIKE', 'IS', 'NULL', 'AS', 'DISTINCT', 'COUNT', 'SUM', 'AVG', 'MIN', 'MAX', 'CASE', 'WHEN', 'THEN', 'ELSE', 'END', 'IF', 'WHILE', 'FOR']
        ...
        function_pattern = '\\b[A-Za-z_][A-Za-z0-9_]*(?=\\s*\\()'
...
class MainTab(QWidget):
        ...
        try:
            ...
            for table in tables:
                ...
                for column in table.get('columns', []):
                    ...
                    column_item.setData(0, Qt.ItemDataRole.UserRole, {'type': 'column', 'name': column['name'], 'table': table['name']})
    ...
    async def _execute_query_async(self, query_text: str) -> None:
        try:
            ...
            result = await self._plugin.execute_query(connection_id=self._current_connection_id, query=query_text, limit=limit)
            self._current_query_result = result
            self.operation_finished.emit()
            self.query_executed.emit(result)
            self.status_changed.emit(f'Query executed: {result.row_count} rows in {result.execution_time_ms}ms')
        except Exception as e:
            self.operation_finished.emit()
    ...
    def _delete_connection(self) -> None:
        if not self._current_connection_id:
            ...
            return
        reply = QMessageBox.question(self, 'Confirm Deletion', 'Are you sure you want to delete this connection?', QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
    ...
    def _delete_query(self) -> None:
        ...
        if not current_item:
            ...
            return
        reply = QMessageBox.question(self, 'Confirm Deletion', f"Are you sure you want to delete the query '{current_item.text()}'?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            query_id = current_item.data(Qt.ItemDataRole.UserRole)
