...
class FieldMappingDialog(QDialog):
    ...
    def _add_mapping(self) -> None:
        original_field, ok = QInputDialog.getText(self, 'Add Mapping', 'Original field name:')
        if not ok or not original_field.strip():
            return
        mapped_field, ok = QInputDialog.getText(self, 'Add Mapping', 'Mapped field name:')
    ...
    def _auto_map_fields(self) -> None:
        QMessageBox.information(self, 'Auto Map', 'Auto-mapping would apply standardization rules to convert field names')
...
class FieldMappingTab(QWidget):
    ...
    async def _load_schema(self) -> None:
        try:
            ...
            for table in tables:
                ...
                table_item.setData(0, Qt.ItemDataRole.UserRole, {'type': 'table', 'name': table['name'], 'connection_id': connection_id})
                for column in table.get('columns', []):
                    column_text = f"{column['name']} ({column.get('type_name', 'UNKNOWN')})"
                    column_item = QTreeWidgetItem([column_text])
                    column_item.setData(0, Qt.ItemDataRole.UserRole, {'type': 'column', 'name': column['name'], 'table': table['name'], 'connection_id': connection_id})
    ...
    def _create_mapping_for_table(self, connection_id: str, table_name: str) -> None:
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if not mappings:
                ...
                return
            asyncio.create_task(self._create_mapping_async(final_connection_id, final_table_name, mappings, description))
    async def _create_mapping_async(self, connection_id: str, table_name: str, mappings: Dict[str, str], description: Optional[str]=None) -> None:
        try:
            self.operation_started.emit('Creating field mapping...')
            await self._plugin.create_field_mapping(connection_id=connection_id, table_name=table_name, mappings=mappings, description=description)
            await self._load_mappings()
            self.operation_finished.emit()
    ...
    def _delete_mapping(self) -> None:
        ...
        if current_row < 0:
            ...
            return
        mapping = self._current_mappings[current_row]
        reply = QMessageBox.question(self, 'Confirm Deletion', f"Are you sure you want to delete the mapping for table '{mapping.get('table_name', 'Unknown')}'?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
