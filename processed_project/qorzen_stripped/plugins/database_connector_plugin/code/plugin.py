...
class DatabaseConnectorPlugin(BasePlugin):
    ...
    async def initialize(self, application_core: Any, **kwargs: Any) -> None:
        ...
        await self._event_bus_manager.publish(event_type='plugin/initialized', source=self.name, payload={'plugin_name': self.name, 'version': self.version, 'has_ui': True})
    async def on_ui_ready(self, ui_integration: Any) -> None:
        ...
        try:
            await set_plugin_state(self.name, PluginLifecycleState.UI_READY)
            await ui_integration.add_menu_item(plugin_id=self.plugin_id, parent_menu='Database', title='Manage Connections', callback=lambda: asyncio.create_task(self._open_connection_manager()))
            ...
            if not self._main_widget:
                self._main_widget = DatabaseConnectorTab(plugin=self, logger=self._logger, concurrency_manager=self._concurrency_manager, event_bus_manager=self._event_bus_manager)
    ...
    async def save_connection(self, config: BaseConnectionConfig) -> str:
        try:
            ...
            await self._event_bus_manager.publish(event_type='database_connector/connection_saved', source=self.name, payload={'connection_id': config.id, 'connection_name': config.name})
            return config.id
        except Exception as e:
    ...
    async def delete_connection(self, connection_id: str) -> bool:
        try:
            ...
            await self._event_bus_manager.publish(event_type='database_connector/connection_deleted', source=self.name, payload={'connection_id': connection_id, 'connection_name': connection_name})
            return True
        except Exception as e:
    ...
    async def get_connector(self, connection_id: str) -> BaseDatabaseConnector:
        try:
            ...
            async with self._connector_locks[connection_id]:
                ...
                connector = get_connector_for_config(config=config, logger=self._logger, security_manager=self._security_manager)
                await connector.connect()
    ...
    async def disconnect(self, connection_id: str) -> bool:
        try:
            ...
            if connector.is_connected:
                ...
                await self._event_bus_manager.publish(event_type='database_connector/disconnected', source=self.name, payload={'connection_id': connection_id})
            return True
        except Exception as e:
            self._logger.error(f'Failed to disconnect: {str(e)}')
            raise PluginError(f'Failed to disconnect: {str(e)}')
    async def test_connection(self, config: BaseConnectionConfig) -> Tuple[bool, Optional[str]]:
        try:
            connector = get_connector_for_config(config=config, logger=self._logger, security_manager=self._security_manager)
    ...
    async def get_saved_queries(self, connection_id: Optional[str]=None) -> Dict[str, SavedQuery]:
        if connection_id:
            return {qid: query for qid, query in self._saved_queries.items() if query.connection_id == connection_id}
        else:
    ...
    async def execute_query(self, connection_id: str, query: str, params: Optional[Dict[str, Any]]=None, limit: Optional[int]=None, mapping_id: Optional[str]=None) -> QueryResult:
        try:
            ...
            self._logger.info(f'Executed query on {connection_id}, returned {result.row_count} rows')
            await self._event_bus_manager.publish(event_type='database_connector/query_executed', source=self.name, payload={'connection_id': connection_id, 'row_count': result.row_count, 'execution_time_ms': result.execution_time_ms})
            return result
        except Exception as e:
            self._logger.error(f'Failed to execute query: {str(e)}')
            raise PluginError(f'Failed to execute query: {str(e)}')
    async def get_field_mappings(self, connection_id: Optional[str]=None, table_name: Optional[str]=None) -> Dict[str, FieldMapping]:
    ...
    async def get_field_mapping(self, mapping_id: str) -> Optional[FieldMapping]:
        return self._field_mappings.get(mapping_id)
    async def save_field_mapping(self, mapping: FieldMapping) -> str:
        try:
            mapping.updated_at = datetime.datetime.now()
            self._field_mappings[mapping.id] = mapping
            await self._save_field_mappings()
            self._logger.info(f'Saved field mapping for {mapping.table_name} on {mapping.connection_id}')
            await self._event_bus_manager.publish(event_type='database_connector/mapping_saved', source=self.name, payload={'mapping_id': mapping.id, 'connection_id': mapping.connection_id, 'table_name': mapping.table_name})
            return mapping.id
        except Exception as e:
    ...
    async def delete_field_mapping(self, mapping_id: str) -> bool:
        try:
            ...
            await self._save_field_mappings()
            self._logger.info(f'Deleted field mapping for {mapping.table_name} on {mapping.connection_id}')
            await self._event_bus_manager.publish(event_type='database_connector/mapping_deleted', source=self.name, payload={'mapping_id': mapping_id, 'connection_id': mapping.connection_id, 'table_name': mapping.table_name})
            return True
        except Exception as e:
    ...
    async def _open_connection_manager(self) -> None:
        if not self._concurrency_manager.is_main_thread():
            await self._concurrency_manager.run_on_main_thread(lambda: asyncio.create_task(self._open_connection_manager()))
            return
        if self._main_widget:
            self._main_widget.open_connection_manager()
    async def _open_query_editor(self) -> None:
        if not self._concurrency_manager.is_main_thread():
            await self._concurrency_manager.run_on_main_thread(lambda: asyncio.create_task(self._open_query_editor()))
            return
        if self._main_widget:
            self._main_widget.switch_to_query_editor()
    async def _open_field_mappings(self) -> None:
        if not self._concurrency_manager.is_main_thread():
            await self._concurrency_manager.run_on_main_thread(lambda: asyncio.create_task(self._open_field_mappings()))
            return
        if self._main_widget:
            self._main_widget.switch_to_mapping_editor()
    async def _open_validation(self) -> None:
        if not self._concurrency_manager.is_main_thread():
            await self._concurrency_manager.run_on_main_thread(lambda: asyncio.create_task(self._open_validation()))
            return
        if self._main_widget:
            self._main_widget.switch_to_validation()
    async def _open_history_manager(self) -> None:
        if not self._concurrency_manager.is_main_thread():
            await self._concurrency_manager.run_on_main_thread(lambda: asyncio.create_task(self._open_history_manager()))
            return
        if self._main_widget:
    ...
    async def _load_settings(self) -> None:
        try:
            if not self._config_manager:
                return
            settings_dict = await self._config_manager.get(f'plugins.{self.name}.settings', {})
            if settings_dict:
    ...
    async def _load_connections(self) -> None:
        try:
            ...
            try:
                file_info = self._file_manager.get_file_info(file_path, 'plugin_data')
                if not file_info:
                    return
            except:
                return
    ...
    async def _on_config_changed(self, event: Any) -> None:
        ...
        await self._load_settings()
        if self._main_widget:
            self._main_widget.handle_config_change(event.payload.get('key'), event.payload.get('value'))
    ...
    def status(self) -> Dict[str, Any]:
        status = {'name': self.name, 'version': self.version, 'initialized': self._initialized, 'connections': {'total': len(self._connections), 'active': len(self._active_connectors)}, 'queries': {'total': len(self._saved_queries)}, 'mappings': {'total': len(self._field_mappings)}, 'ui_active': self._main_widget is not None}
        if self._settings:
            status['settings'] = {'recent_connections': len(self._settings.recent_connections), 'has_default_connection': self._settings.default_connection_id is not None, 'history_enabled': self._settings.history_database_connection_id is not None}
        return status