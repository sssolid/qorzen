...
class DatabaseConnectorPlugin(BasePlugin):
    ...
    async def initialize(self, application_core: Any, **kwargs: Any) -> None:
        ...
        if self._event_bus_manager:
            await self._event_bus_manager.subscribe(event_type='config/changed', callback=self._on_config_changed, subscriber_id=f'{self.name}_config_subscriber')
        ...
        if self._event_bus_manager:
            await self._event_bus_manager.publish(event_type='plugin/initialized', source=self.name, payload={'plugin_name': self.name, 'version': self.version, 'has_ui': True, 'features': ['connection_management', 'query_editor', 'results_export', 'field_mapping', 'data_validation', 'history_tracking']})
    async def on_ui_ready(self, ui_integration: Any) -> None:
        ...
        try:
            ...
            if not self._main_widget:
                self._main_widget = DatabasePluginWidget(plugin=self, logger=self._logger, concurrency_manager=self._concurrency_manager, event_bus_manager=self._event_bus_manager)
            ...
            if self.plugin_info and self.plugin_info.path:
                ...
                if hasattr(self.plugin_info, 'manifest') and hasattr(self.plugin_info.manifest, 'icon_path'):
                    ...
                    if icon_file.exists():
                        icon_arg = str(icon_file)
            await ui_integration.add_page(plugin_id=self.plugin_id, page_component=self._main_widget, icon=icon_arg, title=self.display_name or self.name)
    ...
    async def _setup_menu_items(self, ui_integration: Any) -> None:
        await ui_integration.add_menu_item(plugin_id=self.plugin_id, parent_menu='Database', title='Connection Manager', callback=lambda: asyncio.create_task(self._open_connection_manager()))
        await ui_integration.add_menu_item(plugin_id=self.plugin_id, parent_menu='Database', title='Query Editor', callback=lambda: asyncio.create_task(self._open_query_editor()))
        await ui_integration.add_menu_item(plugin_id=self.plugin_id, parent_menu='Database', title='Field Mappings', callback=lambda: asyncio.create_task(self._open_field_mappings()))
        await ui_integration.add_menu_item(plugin_id=self.plugin_id, parent_menu='Database', title='Data Validation', callback=lambda: asyncio.create_task(self._open_validation()))
        await ui_integration.add_menu_item(plugin_id=self.plugin_id, parent_menu='Database', title='History Manager', callback=lambda: asyncio.create_task(self._open_history_manager()))
    ...
    async def create_connection(self, connection: DatabaseConnection) -> str:
        try:
            ...
            if self._settings:
                ...
                self._settings.recent_connections = self._settings.recent_connections[:self._settings.max_recent_connections]
                await self._save_settings()
            self._logger.info(f'Created connection: {connection.name}')
            if self._event_bus_manager:
                await self._event_bus_manager.publish(event_type='database_connector/connection_created', source=self.name, payload={'connection_id': connection.id, 'connection_name': connection.name})
    ...
    async def update_connection(self, connection: DatabaseConnection) -> None:
        try:
            ...
            if self._event_bus_manager:
                await self._event_bus_manager.publish(event_type='database_connector/connection_updated', source=self.name, payload={'connection_id': connection.id, 'connection_name': connection.name})
        except Exception as e:
    ...
    async def delete_connection(self, connection_id: str) -> bool:
        try:
            ...
            if self._event_bus_manager:
                await self._event_bus_manager.publish(event_type='database_connector/connection_deleted', source=self.name, payload={'connection_id': connection_id, 'connection_name': connection.name})
            return True
        except Exception as e:
    ...
    async def save_query(self, query: SavedQuery) -> str:
        try:
            ...
            if self._event_bus_manager:
                await self._event_bus_manager.publish(event_type='database_connector/query_saved', source=self.name, payload={'query_id': query.id, 'query_name': query.name})
            return query.id
        except Exception as e:
    ...
    async def delete_query(self, query_id: str) -> bool:
        try:
            ...
            if self._event_bus_manager:
                await self._event_bus_manager.publish(event_type='database_connector/query_deleted', source=self.name, payload={'query_id': query_id, 'query_name': query_name})
            return True
        except Exception as e:
            self._logger.error(f'Failed to delete query: {e}')
            return False
    async def execute_query(self, connection_id: str, query: str, parameters: Optional[Dict[str, Any]]=None, limit: Optional[int]=None, apply_mapping: bool=False) -> QueryResult:
        try:
            if connection_id not in self._connections:
                raise PluginError(f'Connection {connection_id} not found')
            connection = self._connections[connection_id]
            result = await self._query_service.execute_query(connection_name=connection.name, query=query, parameters=parameters or {}, limit=limit or (self._settings.query_limit if self._settings else 1000), apply_mapping=apply_mapping)
            query_result = QueryResult(connection_id=connection_id, query=query, parameters=parameters or {}, records=result.get('records', []), columns=result.get('columns', []), row_count=result.get('row_count', 0), execution_time_ms=result.get('execution_time_ms', 0), has_error=result.get('has_error', False), error_message=result.get('error_message'), truncated=result.get('truncated', False), applied_mapping=apply_mapping)
            self._logger.info(f'Executed query on {connection.name}, returned {query_result.row_count} rows')
            if self._event_bus_manager:
                await self._event_bus_manager.publish(event_type='database_connector/query_executed', source=self.name, payload={'connection_id': connection_id, 'row_count': query_result.row_count, 'execution_time_ms': query_result.execution_time_ms})
            return query_result
        except Exception as e:
            self._logger.error(f'Failed to execute query: {e}')
            raise PluginError(f'Failed to execute query: {e}') from e
    async def export_results(self, results: QueryResult, format: ExportFormat, file_path: str, settings: Optional[ExportSettings]=None) -> str:
        try:
            export_settings = settings or (self._settings.export_settings if self._settings else ExportSettings())
            return await self._export_service.export_results(results=results, format=format, file_path=file_path, settings=export_settings)
        except Exception as e:
    ...
    async def get_table_columns(self, connection_id: str, table_name: str, schema: Optional[str]=None) -> List[Dict[str, Any]]:
        try:
            if connection_id not in self._connections:
                raise PluginError(f'Connection {connection_id} not found')
            connection = self._connections[connection_id]
    ...
    async def create_field_mapping(self, connection_id: str, table_name: str, mappings: Dict[str, str], description: Optional[str]=None) -> Dict[str, Any]:
        try:
            if connection_id not in self._connections:
                raise PluginError(f'Connection {connection_id} not found')
            connection = self._connections[connection_id]
            return await self._database_manager.create_field_mapping(connection.name, table_name, mappings, description)
        except Exception as e:
    ...
    async def delete_field_mapping(self, mapping_id: str) -> bool:
        try:
            ...
            return False
    async def get_validation_rules(self, connection_id: Optional[str]=None, table_name: Optional[str]=None) -> List[Dict[str, Any]]:
        try:
    ...
    async def create_validation_rule(self, rule_type: str, connection_id: str, table_name: str, field_name: str, parameters: Dict[str, Any], error_message: str, name: Optional[str]=None, description: Optional[str]=None) -> Dict[str, Any]:
        try:
            if connection_id not in self._connections:
                raise PluginError(f'Connection {connection_id} not found')
            connection = self._connections[connection_id]
            return await self._database_manager.create_validation_rule(rule_type, connection.name, table_name, field_name, parameters, error_message, name, description)
        except Exception as e:
    ...
    async def create_history_schedule(self, connection_id: str, query_id: str, frequency: str, name: str, description: Optional[str]=None, retention_days: int=365) -> Dict[str, Any]:
        try:
            if connection_id not in self._connections:
                raise PluginError(f'Connection {connection_id} not found')
            connection = self._connections[connection_id]
            return await self._database_manager.create_history_schedule(connection.name, query_id, frequency, name, description, retention_days)
        except Exception as e:
    ...
    async def update_settings(self, settings: PluginSettings) -> None:
        try:
            ...
            if self._event_bus_manager:
                await self._event_bus_manager.publish(event_type='database_connector/settings_updated', source=self.name, payload={'settings': settings.model_dump()})
        except Exception as e:
    ...
    def _convert_to_db_config(self, connection: DatabaseConnection) -> DatabaseConnectionConfig:
        return DatabaseConnectionConfig(name=connection.name, db_type=connection.connection_type.value, host=connection.host, port=connection.port, database=connection.database, user=connection.user, password=connection.password, connection_string=connection.connection_string, ssl=connection.ssl, read_only=connection.read_only, pool_size=connection.pool_size, max_overflow=connection.max_overflow, pool_recycle=connection.pool_recycle, connection_timeout=connection.connection_timeout, properties=connection.properties, allowed_tables=connection.allowed_tables, dsn=connection.dsn, jt400_jar_path=connection.jt400_jar_path)
    ...
    async def _save_connections(self) -> None:
        try:
            ...
            for connection in self._connections.values():
                ...
                if 'last_tested' in conn_dict and conn_dict['last_tested'] and hasattr(conn_dict['last_tested'], 'isoformat'):
    ...
    async def _save_saved_queries(self) -> None:
        try:
            ...
            for query in self._saved_queries.values():
                ...
                if 'last_executed' in query_dict and query_dict['last_executed'] and hasattr(query_dict['last_executed'], 'isoformat'):
    ...
    def status(self) -> Dict[str, Any]:
        status = {'name': self.name, 'version': self.version, 'initialized': self._initialized, 'database_manager_available': self._database_manager is not None, 'connections': {'total': len(self._connections), 'active': len([c for c in self._connections.values() if c.is_active])}, 'queries': {'saved': len(self._saved_queries)}, 'ui_active': self._main_widget is not None, 'services': {'export_service': self._export_service is not None, 'query_service': self._query_service is not None}}
        if self._settings:
            status['settings'] = {'default_connection': self._settings.default_connection_id, 'recent_connections': len(self._settings.recent_connections), 'query_limit': self._settings.query_limit, 'auto_limit_queries': self._settings.auto_limit_queries}
        return status