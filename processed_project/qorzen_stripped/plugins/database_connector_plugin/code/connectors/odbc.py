...
class ODBCConnector(BaseDatabaseConnector):
    ...
    async def connect(self) -> None:
        async with self._connect_lock:
            ...
            try:
                ...
                self._logger.info('Successfully connected to database via ODBC', extra={'database': self._config.database, 'dsn': self._config.dsn, 'is_filemaker': self._is_filemaker, 'connection_time_ms': int(self._connection_time * 1000)})
                ...
                if any((keyword in error_msg.lower() for keyword in ['permission', 'access denied', 'authorization', 'login', 'password'])):
                    raise SecurityError(message=f'Security error connecting via ODBC: {sanitized_error}', details={'original_error': sanitized_error})
                else:
                    raise DatabaseError(message=f'Failed to connect to database via ODBC: {sanitized_error}', details={'original_error': sanitized_error})
    async def disconnect(self) -> None:
        ...
        try:
            ...
            raise DatabaseError(message=f'Failed to close ODBC connection: {str(e)}', details={'original_error': str(e)})
    async def execute_query(self, query: str, params: Optional[Dict[str, Any]]=None, limit: Optional[int]=None) -> QueryResult:
        if not self._connected or not self._connection:
            await self.connect()
        if not self._connection:
            raise DatabaseError(message='Not connected to database', details={'connection_id': self._config.id})
        pyodbc = self._pyodbc
        ...
        try:
            ...
            self._logger.debug('Executing ODBC query', extra={'query': sanitized_query, 'limit': limit})
            ...
            self._logger.info('Successfully executed query via ODBC', extra={'record_count': result.row_count, 'execution_time_ms': result.execution_time_ms, 'table': table_name})
            return result
        except Exception as e:
            error_msg = str(e)
            sanitized_error = self._sanitize_error_message(error_msg)
            ...
            if any((keyword in error_msg.lower() for keyword in ['permission', 'access denied', 'privilege'])):
                raise SecurityError(message=f'Security error executing ODBC query: {sanitized_error}', details={'original_error': sanitized_error, 'query': self._sanitize_sql_for_logging(query)})
            else:
                raise DatabaseError(message=f'Failed to execute ODBC query: {sanitized_error}', details={'original_error': sanitized_error, 'query': self._sanitize_sql_for_logging(query)})
        finally:
                        ...
                        try:
                            ...
                            for col_row in col_cursor:
                                ...
                                try:
                                    ...
                                    pass
                                columns.append(ColumnMetadata(name=col_name, type_name=col_type, type_code=col_type_code, precision=precision, scale=scale, nullable=col_nullable, table_name=table_name, remarks=col_remarks))
                            col_cursor.close()
                        except Exception as e:
                            self._logger.warning(f'Error retrieving columns for table {table_name}: {str(e)}')
                        tables.append(TableMetadata(name=table_name, schema=table_schema, type=table_type, remarks=remarks, columns=columns))
            finally:
                cursor.close()
            return tables
        try:
            ...
            raise DatabaseError(message=f'Failed to get tables information: {sanitized_error}', details={'schema': schema})
    async def get_table_columns(self, table_name: str, schema: Optional[str]=None) -> List[ColumnMetadata]:
        ...
        try:
            ...
            raise DatabaseError(message=f'Failed to get column information: {sanitized_error}', details={'table': table_name, 'schema': schema})
    def get_connection_info(self) -> Dict[str, Any]:
        info = {'connected': self._connected, 'connection_id': self._config.id, 'name': self._config.name, 'dsn': self._config.dsn, 'server': self._config.server, 'database': self._config.database, 'username': self._config.username, 'type': 'ODBC', 'is_filemaker': self._is_filemaker, 'read_only': self._config.read_only}
    ...
    def _validate_and_prepare_query(self, query: str, limit: Optional[int]) -> Tuple[str, Optional[str]]:
        ...
        if ' ' not in query:
            ...
            if self._config.read_only and any((write_op in query_upper for write_op in ['INSERT', 'UPDATE', 'DELETE', 'CREATE', 'DROP', 'ALTER', 'TRUNCATE', 'GRANT', 'REVOKE'])):
            ...
            match = re.search('FROM\\s+(\\w+)', query_upper)
            if match:
        ...
        if cursor.description:
            for i, col_desc in enumerate(cursor.description):
                ...
                try:
                    ...
                    pass
                columns.append(ColumnMetadata(name=name, type_name=type_name, type_code=type_code, precision=precision, scale=scale, nullable=nullable))
        return columns