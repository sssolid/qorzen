...
class ColumnSelectionDialog(QDialog):
    def __init__(self, available_columns: List[Dict[str, str]], selected_columns: List[str], parent: Optional[QWidget]=None) -> None:
...
class ExportOptionsDialog(QDialog):
    def __init__(self, format_type: str, current_count: int, total_count: int, parent: Optional[QWidget]=None) -> None:
        super().__init__(parent)
...
class DataTableWidget(QWidget):
    ...
    def __init__(self, database_handler: DatabaseHandler, event_bus_manager: EventBusManager, logger: logging.Logger, parent: Optional[QWidget]=None) -> None:
        super().__init__(parent)
    ...
    async def _subscribe_to_events(self) -> None:
        await self._event_bus_manager.subscribe(event_type=VCdbEventType.query_results(), callback=self._on_query_results, subscriber_id=self._callback_id)
    ...
    def execute_query(self, filter_panels: List[Dict[str, List[int]]]) -> None:
        ...
        asyncio.create_task(self._event_bus_manager.publish(event_type=VCdbEventType.query_execute(), source='vcdb_explorer', payload={'filter_panels': filter_panels, 'columns': self._selected_columns, 'page': self._current_page, 'page_size': self._page_size, 'sort_by': self._sort_column, 'sort_desc': self._sort_descending, 'table_filters': {}, 'callback_id': self._callback_id}))
    ...
    def _on_query_progress(self, current: int, total: int) -> None:
        if self._overlay_progress:
            if total > 0:
                self._overlay_progress.set_progress(current, total, f'Processing {current} of {total}...')
            else:
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if new_columns != self._selected_columns:
                ...
                if self._model.rowCount() > 0:
                    self._run_query()
    async def _export_to_file(self, export_type: str, data: List[Dict[str, Any]], file_path: str) -> None:
        try:
            if export_type == 'csv':
                await self._exporter.export_csv(data=data, columns=self._selected_columns, column_map=self._column_map, file_path=file_path)
            elif export_type == 'excel' and EXCEL_AVAILABLE:
                await self._exporter.export_excel(data=data, columns=self._selected_columns, column_map=self._column_map, file_path=file_path)
            QMessageBox.information(self, 'Export Complete', f'Successfully exported {len(data)} rows to {export_type.upper()} file.')
        except Exception as e:
            self._logger.error(f'Error exporting data: {str(e)}')
            QMessageBox.critical(self, 'Export Error', f'Error exporting data: {str(e)}')
    async def _export_all_to_file(self, export_type: str, file_path: str, progress_dialog: QProgressDialog) -> None:
        try:
            async def update_progress(current: int, total: int) -> Optional[bool]:
                ...
                return None
            rows_exported = await self._exporter.export_all_data(database_callback=lambda filter_panels, columns, page, page_size, sort_by, sort_desc, table_filters: self._database_handler.execute_query(filter_panels=filter_panels, columns=columns, page=page, page_size=page_size, sort_by=sort_by, sort_desc=sort_desc, table_filters=table_filters), filter_panels=self._current_filter_panels, columns=self._selected_columns, column_map=self._column_map, file_path=file_path, format_type=export_type, sort_by=self._sort_column, sort_desc=self._sort_descending, table_filters={}, progress_callback=update_progress)
            if not progress_dialog.wasCanceled():
                QMessageBox.information(self, 'Export Complete', f'Successfully exported {rows_exported} rows to {export_type.upper()} file.')
        except Exception as e:
            self._logger.error(f'Error exporting data: {str(e)}')
            QMessageBox.critical(self, 'Export Error', f'Error exporting data: {str(e)}')
        ...
        if self._total_count == 0:
            ...
            if self._filter_group.isChecked() and self._table_filters:
                ...
                if filtered_count < end_idx - start_idx + 1:
                    self._count_label.setText(f'Showing {filtered_count} of {self._total_count} rows (filtered)')
                else:
    ...
    def _copy_selected_rows(self) -> None:
        ...
        for idx in selected_indexes:
            ...
            row_data = []
            for col_idx, col_id in enumerate(self._selected_columns):
                cell_value = str(self._model.data(self._model.index(source_row, col_idx), Qt.ItemDataRole.DisplayRole) or '')
