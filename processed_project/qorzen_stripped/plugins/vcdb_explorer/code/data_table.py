...
class DataTableWidget(QWidget):
    ...
    def __init__(self, database_handler: DatabaseHandler, event_bus: EventBusManager, logger: logging.Logger, parent: Optional[QWidget]=None) -> None:
        super().__init__(parent)
        ...
        self._event_bus.subscribe(event_type=VCdbEventType.query_results(), callback=self._on_query_results, subscriber_id=self._callback_id)
    ...
    def execute_query(self, filter_panels: List[Dict[str, List[int]]]) -> None:
        ...
        self._query_thread = threading.Thread(target=self._run_query_in_thread, args=(filter_panels,), daemon=True)
    ...
    def _run_query_in_thread(self, filter_panels: List[Dict[str, List[int]]]) -> None:
        try:
            ...
            results, total_count = self._database_handler.execute_query(filter_panels=filter_panels, columns=self._selected_columns, page=self._current_page, page_size=self._page_size, sort_by=self._sort_column, sort_desc=self._sort_descending, table_filters=self._table_filters)
    ...
    def _export_data(self, export_type: str) -> None:
        ...
        try:
            if not export_all:
                ...
                if export_type == 'csv':
                    self._exporter.export_csv(data=data, columns=self._selected_columns, column_map=self._column_map, file_path=file_path)
                elif export_type == 'excel' and EXCEL_AVAILABLE:
                    self._exporter.export_excel(data=data, columns=self._selected_columns, column_map=self._column_map, file_path=file_path)
    ...
    def _copy_selected_rows(self) -> None:
        ...
        for idx in selected_indexes:
            ...
            row_data = []
            for col_idx, col_id in enumerate(self._selected_columns):
                cell_value = str(self._model.data(self._model.index(row_idx, col_idx), Qt.ItemDataRole.DisplayRole) or '')
