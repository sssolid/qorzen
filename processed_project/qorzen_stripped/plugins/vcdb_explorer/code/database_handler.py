...
class DatabaseHandler:
    ...
    def __init__(self, database_manager: DatabaseManager, event_bus_manager: EventBusManager, task_manager: TaskManager, concurrency_manager: ConcurrencyManager, logger: logging.Logger) -> None:
        ...
        self._filter_map = {'year': (Year, 'year_id'), 'make': (Make, 'make_name'), 'model': (Model, 'model_name'), 'submodel': (SubModel, 'sub_model_name'), 'region': (Region, 'region_name'), 'drivetype': (DriveType, 'drive_type_name'), 'vehicletype': (VehicleType, 'vehicle_type_name'), 'vehicletypegroup': (VehicleTypeGroup, 'vehicle_type_group_name'), 'publicationstage': (PublicationStage, 'publication_stage_name'), 'class': (Class, 'class_name'), 'wheelbase': (WheelBase, 'wheel_base'), 'bedtype': (BedType, 'bed_type_name'), 'bedlength': (BedLength, 'bed_length'), 'bodytype': (BodyType, 'body_type_name'), 'bodynumdoors': (BodyNumDoors, 'body_num_doors'), 'braketype': (BrakeType, 'brake_type_name'), 'brakesystem': (BrakeSystem, 'brake_system_name'), 'brakeabs': (BrakeABS, 'brake_abs_name'), 'engineblock': (EngineBlock, 'cylinders'), 'fueltypename': (FuelType, 'fuel_type_name')}
    ...
    async def initialize(self) -> None:
        try:
            await self._event_bus_manager.subscribe(event_type=VCdbEventType.filter_changed(), callback=self._on_filter_changed, subscriber_id='vcdb_explorer_handler')
            await self._event_bus_manager.subscribe(event_type=VCdbEventType.query_execute(), callback=self._on_query_execute, subscriber_id='vcdb_explorer_handler')
    ...
    async def configure(self, host: str, port: int, database: str, user: str, password: str, db_type: str='postgresql', pool_size: int=5, max_overflow: int=10, pool_recycle: int=3600, echo: bool=False) -> None:
        ...
        try:
            ...
            if has_connection:
                ...
                return
            self._connection_config = DatabaseConnectionConfig(name=self.CONNECTION_NAME, db_type=db_type, host=host, port=port, database=database, user=user, password=password, pool_size=pool_size, max_overflow=max_overflow, pool_recycle=pool_recycle, echo=echo)
    ...
    async def _on_filter_changed(self, event: Event) -> None:
        ...
        self._logger.debug(f'Filter changed event: panel={filter_panel_id}, type={filter_type}, values={values}, auto_populate={auto_populate}')
        if auto_populate and current_filters:
            try:
                task_id = await self._task_manager.submit_async_task(func=self._refresh_filters, filter_panel_id=filter_panel_id, filter_type=filter_type, current_filters=current_filters, name=f'filter_refresh_{filter_panel_id}_{filter_type}', category=TaskCategory.PLUGIN, plugin_id='vcdb_explorer')
    ...
    async def _on_query_execute(self, event: Event) -> None:
        ...
        try:
            ...
            task_id = await self._task_manager.submit_async_task(func=self._execute_query, filter_panels=filter_panels, columns=columns, page=page, page_size=page_size, sort_by=sort_by, sort_desc=sort_desc, table_filters=table_filters, callback_id=callback_id, cancellation_token=query_token, name=f'vcdb_query_execute', category=TaskCategory.PLUGIN, plugin_id='vcdb_explorer')
            ...
            await self._event_bus_manager.publish(event_type=VCdbEventType.query_results(), source='vcdb_explorer', payload={'results': [], 'total_count': 0, 'error': str(e), 'callback_id': callback_id})
    async def cancel_query(self, callback_id: str) -> bool:
        ...
        if query_token in self._query_cancellation_tokens:
            ...
            return True
        return False
    async def _refresh_filters(self, filter_panel_id: str, filter_type: str, current_filters: Dict[str, List[int]], progress_reporter: Any=None, cancellation_token: Optional[str]=None) -> Dict[str, Any]:
        ...
        try:
            ...
            to_refresh = [ft for ft in self._filter_map.keys() if ft not in {filter_type, 'year', 'year_range'}]
            ...
            await self._event_bus_manager.publish(event_type=VCdbEventType.filters_refreshed(), source='vcdb_explorer', payload={'panel_id': filter_panel_id, 'filter_values': results})
            if progress_reporter:
    ...
    async def _execute_query(self, filter_panels: List[Dict[str, List[int]]], columns: List[str], page: int, page_size: int, sort_by: Optional[str], sort_desc: bool, table_filters: Dict[str, Any], callback_id: Optional[str], progress_reporter: Any=None, cancellation_token: Optional[str]=None) -> None:
        ...
        try:
            ...
            async with self._db_manager.async_session(self.CONNECTION_NAME) as session:
                results, total_count = await self._execute_query_async(session, filter_panels, columns, page, page_size, sort_by, sort_desc, table_filters, progress_reporter, cancellation_token)
            if cancellation_token and cancellation_token in self._query_cancellation_tokens:
                if self._query_cancellation_tokens[cancellation_token].is_set():
                    ...
                    await self._event_bus_manager.publish(event_type=VCdbEventType.query_results(), source='vcdb_explorer', payload={'results': [], 'total_count': 0, 'cancelled': True, 'callback_id': callback_id})
                    return
            if progress_reporter:
            ...
            await self._event_bus_manager.publish(event_type=VCdbEventType.query_results(), source='vcdb_explorer', payload={'results': results, 'total_count': total_count, 'callback_id': callback_id})
            if progress_reporter:
    ...
    async def _execute_query_async(self, session: AsyncSession, filter_panels: List[Dict[str, List[int]]], columns: List[str], page: int, page_size: int, sort_by: Optional[str], sort_desc: bool, table_filters: Dict[str, Any], progress_reporter: Any=None, cancellation_token: Optional[str]=None) -> Tuple[List[Dict[str, Any]], int]:
        if not self._initialized:
            raise DatabaseHandlerError('DatabaseHandler not initialized')
        try:
            self._logger.debug(f'Executing async query: panels={len(filter_panels)}, columns={columns}, page={page}, page_size={page_size}')
    ...
    async def get_filter_values(self, filter_type: str, current_filters: Dict[str, List[int]], exclude_filters: Optional[Set[str]]=None) -> List[Dict[str, Any]]:
        if not self._initialized:
            raise DatabaseHandlerError('DatabaseHandler not initialized')
        ...
        async with self._db_manager.async_session(self.CONNECTION_NAME) as session:
            if filter_type == 'year':
                query = select(Year.year_id.label('id'), Year.year_id.label('name'), func.count(Year.year_id).label('count')).select_from(Year).join(BaseVehicle, BaseVehicle.year_id == Year.year_id).join(Vehicle, Vehicle.base_vehicle_id == BaseVehicle.base_vehicle_id)
                query = query.group_by(Year.year_id).order_by(Year.year_id)
            elif filter_type == 'make':
                query = select(Make.make_id.label('id'), Make.make_name.label('name'), func.count(Make.make_id).label('count')).select_from(Make).join(BaseVehicle, BaseVehicle.make_id == Make.make_id).join(Vehicle, Vehicle.base_vehicle_id == BaseVehicle.base_vehicle_id)
                query = query.group_by(Make.make_id, Make.make_name).order_by(Make.make_name)
            elif filter_type == 'model':
                query = select(Model.model_id.label('id'), Model.model_name.label('name'), func.count(Model.model_id).label('count')).select_from(Model).join(BaseVehicle, BaseVehicle.model_id == Model.model_id).join(Vehicle, Vehicle.base_vehicle_id == BaseVehicle.base_vehicle_id)
                query = query.group_by(Model.model_id, Model.model_name).order_by(Model.model_name)
            elif filter_type == 'submodel':
                query = select(SubModel.sub_model_id.label('id'), SubModel.sub_model_name.label('name'), func.count(SubModel.sub_model_id).label('count')).select_from(SubModel).join(Vehicle, Vehicle.sub_model_id == SubModel.sub_model_id)
                query = query.group_by(SubModel.sub_model_id, SubModel.sub_model_name).order_by(SubModel.sub_model_name)
            elif filter_type == 'region':
                query = select(Region.region_id.label('id'), Region.region_name.label('name'), func.count(Region.region_id).label('count')).select_from(Region).join(Vehicle, Vehicle.region_id == Region.region_id)
                ...
                query = select(dt.drive_type_id.label('id'), dt.drive_type_name.label('name'), func.count(dt.drive_type_id).label('count')).select_from(Vehicle).join(vtdt, Vehicle.vehicle_id == vtdt.vehicle_id).join(dt, vtdt.drive_type_id == dt.drive_type_id)
                ...
                query = select(wb.wheel_base_id.label('id'), wb.wheel_base.label('name'), func.count(wb.wheel_base_id).label('count')).select_from(Vehicle).join(vtwb, Vehicle.vehicle_id == vtwb.vehicle_id).join(wb, vtwb.wheel_base_id == wb.wheel_base_id)
                ...
                bt = aliased(BedType)
                query = select(bt.bed_type_id.label('id'), bt.bed_type_name.label('name'), func.count(bt.bed_type_id).label('count')).select_from(Vehicle).join(vtbc, Vehicle.vehicle_id == vtbc.vehicle_id).join(bc, vtbc.bed_config_id == bc.bed_config_id).join(bt, bc.bed_type_id == bt.bed_type_id)
                ...
                query = select(bl.bed_length_id.label('id'), bl.bed_length.label('name'), func.count(bl.bed_length_id).label('count')).select_from(Vehicle).join(vtbc, Vehicle.vehicle_id == vtbc.vehicle_id).join(bc, vtbc.bed_config_id == bc.bed_config_id).join(bl, bc.bed_length_id == bl.bed_length_id)
                ...
                bt = aliased(BodyType)
                query = select(bsc.body_type_id.label('id'), bt.body_type_name.label('name'), func.count(bt.body_type_id).label('count')).select_from(Vehicle).join(vtbc, Vehicle.vehicle_id == vtbc.vehicle_id).join(bsc, vtbc.body_style_config_id == bsc.body_style_config_id).join(bt, bt.body_type_id == bsc.body_type_id)
                ...
                query = select(bsc.body_num_doors_id.label('id'), bnd.body_num_doors.label('name'), func.count(bnd.body_num_doors_id).label('count')).select_from(Vehicle).join(vtbc, Vehicle.vehicle_id == vtbc.vehicle_id).join(bsc, vtbc.body_style_config_id == bsc.body_style_config_id).join(bnd, bnd.body_num_doors_id == bsc.body_type_id)
                ...
                query = select(eb.engine_block_id.label('id'), eb.cylinders.label('name'), func.count(eb.engine_block_id).label('count')).select_from(Vehicle).join(vtec, Vehicle.vehicle_id == vtec.vehicle_id).join(ec, vtec.engine_config_id == ec.engine_config_id).join(eb, ec.engine_block_id == eb.engine_block_id)
                query = query.group_by(eb.engine_block_id, eb.cylinders).order_by(eb.cylinders)
            elif filter_type == 'fueltypename':
                vtec = aliased(VehicleToEngineConfig)
                ec = aliased(EngineConfig2)
                ft = aliased(FuelType)
                query = select(ft.fuel_type_id.label('id'), ft.fuel_type_name.label('name'), func.count(ft.fuel_type_id).label('count')).select_from(Vehicle).join(vtec, Vehicle.vehicle_id == vtec.vehicle_id).join(ec, vtec.engine_config_id == ec.engine_config_id).join(ft, ec.fuel_type_id == ft.fuel_type_id)
                ...
                query = select(vtg.vehicle_type_group_id.label('id'), vtg.vehicle_type_group_name.label('name'), func.count(vtg.vehicle_type_group_id).label('count')).select_from(Vehicle).join(bv, Vehicle.base_vehicle_id == bv.base_vehicle_id).join(m, bv.model_id == m.model_id).join(vt, m.vehicle_type_id == vt.vehicle_type_id).join(vtg, vt.vehicle_type_group_id == vtg.vehicle_type_group_id).group_by(vtg.vehicle_type_group_id, vtg.vehicle_type_group_name).order_by(vtg.vehicle_type_group_name)
            else:
                pk_column = getattr(model_class, model_class.__table__.primary_key.columns.keys()[0])
                name_column = getattr(model_class, attr_name)
                query = select(pk_column.label('id'), name_column.label('name'), func.count(pk_column).label('count')).select_from(Vehicle)
                if model_class == VehicleType:
                    query = query.join(BaseVehicle, Vehicle.base_vehicle_id == BaseVehicle.base_vehicle_id).join(Model, BaseVehicle.model_id == Model.model_id).join(VehicleType, Model.vehicle_type_id == VehicleType.vehicle_type_id)
                elif model_class == PublicationStage:
                    query = query.join(PublicationStage, Vehicle.publication_stage_id == PublicationStage.publication_stage_id)
                elif model_class == BrakeType:
                    vtbc = aliased(VehicleToBrakeConfig)
                    bc = aliased(BrakeConfig)
                    query = query.join(vtbc, Vehicle.vehicle_id == vtbc.vehicle_id).join(bc, vtbc.brake_config_id == bc.brake_config_id).join(BrakeType, or_(bc.front_brake_type_id == BrakeType.brake_type_id, bc.rear_brake_type_id == BrakeType.brake_type_id))
                elif model_class == BrakeSystem:
                    vtbc = aliased(VehicleToBrakeConfig)
                    bc = aliased(BrakeConfig)
                    query = query.join(vtbc, Vehicle.vehicle_id == vtbc.vehicle_id).join(bc, vtbc.brake_config_id == bc.brake_config_id).join(BrakeSystem, bc.brake_system_id == BrakeSystem.brake_system_id)
                elif model_class == BrakeABS:
                    vtbc = aliased(VehicleToBrakeConfig)
                    bc = aliased(BrakeConfig)
                    query = query.join(vtbc, Vehicle.vehicle_id == vtbc.vehicle_id).join(bc, vtbc.brake_config_id == bc.brake_config_id).join(BrakeABS, bc.brake_abs_id == BrakeABS.brake_abs_id)
                elif model_class == Class:
                    vtc = aliased(VehicleToClass)
                    query = query.join(vtc, Vehicle.vehicle_id == vtc.vehicle_id).join(Class, vtc.class_id == Class.class_id)
                query = query.group_by(pk_column, name_column).order_by(name_column)
            query = self._apply_filters(query, current_filters, exclude_filters, target_model=model_class)
    ...
    def _apply_filters(self, query: Any, filters: Dict[str, List[int]], exclude_filters: Set[str], target_model: Any=None) -> Any:
        if not filters:
        ...
        def _contains_base_vehicle(clause) -> bool:
            ...
            if isinstance(clause, AliasedInsp) and clause.original is BaseVehicle:
                return True
            return False
        has_base_vehicle_join = any((_contains_base_vehicle(f) for f in query.get_final_froms()))
            ...
            try:
                if filter_type == 'year':
                    ...
                    if model_class == Year:
                        ...
                        has_year_join = False
                        for clause in getattr(query, '_from_obj', []):
                            if isinstance(clause, Join) and hasattr(clause, 'right') and (clause.right == Year.__table__):
                                has_year_join = True
                                break
                        ...
                        has_make_join = False
                        for clause in getattr(query, '_from_obj', []):
                            if isinstance(clause, Join) and hasattr(clause, 'right') and (clause.right == Make.__table__):
                                has_make_join = True
                                break
                        ...
                        has_model_join = False
                        for clause in getattr(query, '_from_obj', []):
                            if isinstance(clause, Join) and hasattr(clause, 'right') and (clause.right == Model.__table__):
                                has_model_join = True
                                break
                        if not has_model_join:
                            query = query.join(Model, BaseVehicle.model_id == Model.model_id)
                        ...
                        self._logger.warning(f'Unimplemented filter type in map: {filter_type} ({model_class.__name__}.{attr_name})')
                else:
    ...
    def _build_columns_query(self, vehicle_ids: Any, columns: List[str]) -> Any:
        ...
        query = query.add_columns(BaseVehicle.year_id.label('year')).join(BaseVehicle, Vehicle.base_vehicle_id == BaseVehicle.base_vehicle_id, isouter=True)
        query = query.add_columns(Make.make_id.label('make_id'), Make.make_name.label('make')).join(Make, BaseVehicle.make_id == Make.make_id, isouter=True)
        query = query.add_columns(Model.model_id.label('model_id'), Model.model_name.label('model')).join(Model, BaseVehicle.model_id == Model.model_id, isouter=True)
        query = query.add_columns(SubModel.sub_model_id.label('submodel_id'), SubModel.sub_model_name.label('submodel')).join(SubModel, Vehicle.sub_model_id == SubModel.sub_model_id, isouter=True)
        for column in columns:
            if column == 'region':
                query = query.add_columns(Region.region_id.label('region_id'), Region.region_name.label('region')).outerjoin(Region, Vehicle.region_id == Region.region_id)
            elif column == 'vehicle_type':
                query = query.add_columns(VehicleType.vehicle_type_id.label('vehicle_type_id'), VehicleType.vehicle_type_name.label('vehicle_type')).outerjoin(VehicleType, Model.vehicle_type_id == VehicleType.vehicle_type_id)
            elif column == 'drive_type':
                vtdt = aliased(VehicleToDriveType)
                dt = aliased(DriveType)
                query = query.add_columns(dt.drive_type_id.label('drive_type_id'), dt.drive_type_name.label('drive_type')).outerjoin(vtdt, Vehicle.vehicle_id == vtdt.vehicle_id).outerjoin(dt, vtdt.drive_type_id == dt.drive_type_id)
        return query
    ...
    def _apply_sorting(self, query: Any, sort_by: str, sort_desc: bool) -> Any:
        sort_map = {'vehicle_id': Vehicle.vehicle_id, 'year': BaseVehicle.year_id, 'make': Make.make_name, 'model': Model.model_name, 'submodel': SubModel.sub_model_name, 'region': Region.region_name}
    ...
    def get_available_columns(self) -> List[Dict[str, str]]:
        ...
        columns = [{'id': 'vehicle_id', 'name': 'Vehicle ID'}, {'id': 'year', 'name': 'Year'}, {'id': 'make', 'name': 'Make'}, {'id': 'model', 'name': 'Model'}, {'id': 'submodel', 'name': 'Submodel'}, {'id': 'region', 'name': 'Region'}, {'id': 'vehicle_type', 'name': 'Vehicle Type'}, {'id': 'drive_type', 'name': 'Drive Type'}, {'id': 'brake_system', 'name': 'Brake System'}, {'id': 'body_type', 'name': 'Body Type'}, {'id': 'engine', 'name': 'Engine'}, {'id': 'transmission', 'name': 'Transmission'}, {'id': 'wheel_base', 'name': 'Wheel Base'}, {'id': 'bed_type', 'name': 'Bed Type'}, {'id': 'bed_length', 'name': 'Bed Length'}, {'id': 'bed_length_metric', 'name': 'Bed Length (Metric)'}]
    ...
    def get_available_filters(self) -> List[Dict[str, Any]]:
        ...
        filters = [{'id': 'year', 'name': 'Year', 'mandatory': True}, {'id': 'year_range', 'name': 'Year Range', 'mandatory': True}, {'id': 'make', 'name': 'Make', 'mandatory': True}, {'id': 'model', 'name': 'Model', 'mandatory': True}, {'id': 'submodel', 'name': 'Submodel', 'mandatory': True}, {'id': 'region', 'name': 'Region', 'mandatory': False}, {'id': 'drivetype', 'name': 'Drive Type', 'mandatory': False}, {'id': 'vehicletype', 'name': 'Vehicle Type', 'mandatory': False}, {'id': 'brakesystem', 'name': 'Brake System', 'mandatory': False}, {'id': 'bodytype', 'name': 'Body Type', 'mandatory': False}, {'id': 'wheelbase', 'name': 'Wheel Base', 'mandatory': False}, {'id': 'class', 'name': 'Vehicle Class', 'mandatory': False}, {'id': 'bedtype', 'name': 'Bed Type', 'mandatory': False}, {'id': 'bedlength', 'name': 'Bed Length', 'mandatory': False}, {'id': 'bodynumdoors', 'name': 'Body Number of Doors', 'mandatory': False}, {'id': 'braketype', 'name': 'Brake Type', 'mandatory': False}, {'id': 'brakeabs', 'name': 'Brake ABS', 'mandatory': False}, {'id': 'engineblock', 'name': 'Engine Block', 'mandatory': False}, {'id': 'fueltypename', 'name': 'Fuel Type', 'mandatory': False}]
