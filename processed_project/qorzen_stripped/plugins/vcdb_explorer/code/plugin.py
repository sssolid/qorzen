...
class VCdbExplorerWidget(QWidget):
    def __init__(self, database_handler: DatabaseHandler, event_bus_manager: EventBusManager, concurrency_manager: ConcurrencyManager, task_manager: TaskManager, logger: logging.Logger, export_settings: Dict[str, Any], parent: Optional[QWidget]=None) -> None:
    ...
    async def _subscribe_to_events(self) -> None:
        await self._event_bus_manager.subscribe(event_type=VCdbEventType.filters_refreshed(), callback=self._on_filters_refreshed, subscriber_id='vcdb_explorer_widget')
    def _create_ui_components(self) -> None:
        self._main_splitter = QSplitter(Qt.Orientation.Vertical)
        self._filter_panel_manager = FilterPanelManager(self._database_handler, self._event_bus_manager, self._logger, max_panels=self._export_settings.get('max_filter_panels', 5))
        ...
        self._data_table = DataTableWidget(self._database_handler, self._event_bus_manager, self._logger, self)
    ...
    async def _on_filters_refreshed(self, event: Any) -> None:
        if not self._concurrency_manager.is_main_thread():
            await self._concurrency_manager.run_on_main_thread(lambda: asyncio.create_task(self._on_filters_refreshed(event)))
            return
    ...
    def _execute_query(self) -> None:
        ...
        if not any((panel for panel in filter_panels if panel)):
            if QMessageBox.question(self, 'No Filters', "You haven't set any filters. This could return a large number of results. Continue?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No) != QMessageBox.StandardButton.Yes:
                return
    ...
    async def _execute_query_async(self, filter_panels: List[Dict[str, List[int]]]) -> None:
        try:
            ...
            if not self._concurrency_manager.is_main_thread():
                await self._concurrency_manager.run_on_main_thread(lambda: QMessageBox.critical(self, 'Query Error', f'Error executing query: {str(e)}'))
            else:
...
class VCdbExplorerPlugin(BasePlugin):
    ...
    async def initialize(self, application_core: Any, **kwargs: Any) -> None:
        ...
        if not database_manager:
            ...
            try:
                self._database_handler = DatabaseHandler(database_manager, self._event_bus_manager, self._task_manager, self._concurrency_manager, self._logger)
                await self._database_handler.initialize()
                await self._database_handler.configure(self._db_config.get('host', 'localhost'), self._db_config.get('port', 5432), self._db_config.get('database', 'vcdb'), self._db_config.get('user', 'postgres'), self._db_config.get('password', ''))
                ...
                self._database_handler = None
                self._connection_registered = False
        await self._event_bus_manager.subscribe(event_type='vcdb_explorer:log_message', callback=self._on_log_message, subscriber_id='vcdb_explorer_plugin')
    ...
    async def _load_config(self) -> None:
        if not self._config_manager:
            return
        self._db_config = {'host': await self._config_manager.get(f'plugins.{self.name}.database.host', 'localhost'), 'port': await self._config_manager.get(f'plugins.{self.name}.database.port', 5432), 'database': await self._config_manager.get(f'plugins.{self.name}.database.name', 'vcdb'), 'user': await self._config_manager.get(f'plugins.{self.name}.database.user', 'postgres'), 'password': await self._config_manager.get(f'plugins.{self.name}.database.password', '')}
        self._ui_config = {'max_filter_panels': await self._config_manager.get(f'plugins.{self.name}.ui.max_filter_panels', 5), 'default_page_size': await self._config_manager.get(f'plugins.{self.name}.ui.default_page_size', 100)}
        self._export_config = {'max_rows': await self._config_manager.get(f'plugins.{self.name}.export.max_rows', 0)}
        if self._logger:
    ...
    async def on_ui_ready(self, ui_integration: UIIntegration) -> None:
        ...
        try:
            await set_plugin_state(self.name, PluginLifecycleState.UI_READY)
            await ui_integration.add_menu_item(plugin_id=self.plugin_id, parent_menu='VCdb', title='Run Query', callback=lambda: asyncio.create_task(self._run_query()))
            await ui_integration.add_menu_item(plugin_id=self.plugin_id, parent_menu='VCdb', title='Refresh Filters', callback=lambda: asyncio.create_task(self._refresh_filters()))
            await ui_integration.add_menu_item(plugin_id=self.plugin_id, parent_menu='VCdb', title='Documentation', callback=lambda: asyncio.create_task(self._open_documentation()))
            await ui_integration.add_menu_item(plugin_id=self.plugin_id, parent_menu='VCdb', title='Configuration', callback=lambda: asyncio.create_task(self._open_configuration()))
            try:
                if not self._main_widget:
                    self._main_widget = VCdbExplorerWidget(self._database_handler, self._event_bus_manager, self._concurrency_manager, self._task_manager, self._logger, self._export_config, None)
    ...
    async def _refresh_filters(self) -> None:
        if not self._concurrency_manager.is_main_thread():
            await self._concurrency_manager.run_on_main_thread(lambda: asyncio.create_task(self._refresh_filters()))
            return
    ...
    async def _run_query(self) -> None:
        if not self._concurrency_manager.is_main_thread():
            await self._concurrency_manager.run_on_main_thread(lambda: asyncio.create_task(self._run_query()))
            return
    ...
    async def _open_documentation(self) -> None:
        if not self._concurrency_manager.is_main_thread():
            await self._concurrency_manager.run_on_main_thread(lambda: asyncio.create_task(self._open_documentation()))
            return
        if self._logger:
            self._logger.info('Documentation requested')
        QMessageBox.information(None, 'Documentation', 'The VCdb Explorer plugin provides an interface to query and explore the Vehicle Component Database.\n\nFor more information, please refer to the online documentation.')
    async def _open_configuration(self) -> None:
        if not self._concurrency_manager.is_main_thread():
            await self._concurrency_manager.run_on_main_thread(lambda: asyncio.create_task(self._open_configuration()))
            return
        QMessageBox.information(None, 'Configuration', 'To configure the VCdb Explorer plugin, edit the configuration file and restart the application.')
