...
class VCdbExplorerWidget(QWidget):
    def __init__(self, database_handler: DatabaseHandler, event_bus: EventBusManager, thread_manager: ThreadManager, logger: logging.Logger, export_settings: Dict[str, Any], parent: Optional[QWidget]=None) -> None:
        ...
        self._event_bus.subscribe(event_type=VCdbEventType.filters_refreshed(), callback=self._on_filters_refreshed, subscriber_id='vcdb_explorer_widget')
    ...
    def _create_ui_components(self) -> None:
        ...
        self._filter_panel_manager = FilterPanelManager(self._database_handler, self._event_bus, self._logger, max_panels=self._export_settings.get('max_filter_panels', 5))
        ...
        try:
            ...
            if not any((panel for panel in filter_panels if panel)):
                if QMessageBox.question(self, 'No Filters', "You haven't set any filters. This could return a large number of results. Continue?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No) != QMessageBox.StandardButton.Yes:
                    ...
                    return
            self._thread_manager.submit_task(self._database_handler._execute_query_thread, filter_panels=filter_panels, columns=self._data_table.get_selected_columns(), page=1, page_size=self._data_table.get_page_size(), sort_by=None, sort_desc=False, table_filters={}, callback_id=self._data_table.get_callback_id(), name=f'query_execution', submitter='vcdb_explorer', result_handler=self._handle_query_result)
...
class VCdbExplorerPlugin(BasePlugin):
    ...
    def initialize(self, event_bus: EventBusManager, logger_provider: LoggingManager, config_provider: ConfigManager, file_manager: FileManager, thread_manager: ThreadManager, database_manager: DatabaseManager, remote_services_manager: RemoteServicesManager, security_manager: SecurityManager, api_manager: APIManager, cloud_manager: CloudManager, **kwargs: Any) -> None:
        super().initialize(event_bus, logger_provider, config_provider, file_manager, thread_manager, database_manager, remote_services_manager, security_manager, api_manager, cloud_manager, **kwargs)
        ...
        if not database_manager:
            ...
            try:
                if self._thread_manager.is_main_thread():
                    ...
                    self._thread_manager.execute_on_main_thread_sync(self._init_database_connection, database_manager)
            except Exception as e:
                self._logger.error(f'Failed to initialize database connection: {str(e)}')
                self._database_handler = None
                self._connection_registered = False
        self._event_bus.subscribe(event_type='vcdb_explorer:log_message', callback=self._on_log_message, subscriber_id='vcdb_explorer_plugin')
    ...
    def _init_database_connection(self, database_manager: DatabaseManager) -> None:
        try:
            self._database_handler = DatabaseHandler(database_manager, self._event_bus, self._thread_manager, self._logger)
            existing_connections = database_manager.get_connection_names()
            if DatabaseHandler.CONNECTION_NAME not in existing_connections:
                try:
                    self._database_handler.configure(self._db_config.get('host', 'localhost'), self._db_config.get('port', 5432), self._db_config.get('database', 'vcdb'), self._db_config.get('user', 'postgres'), self._db_config.get('password', ''))
                    self._connection_registered = True
                except Exception as e:
                    self._logger.error(f'Failed to configure database connection: {str(e)}')
                    self._connection_registered = False
            else:
                self._logger.warning(f'Connection "{DatabaseHandler.CONNECTION_NAME}" already exists, reusing existing connection')
                self._connection_registered = True
        except Exception as e:
    ...
    def _load_config(self) -> None:
        if not self._config:
            return
        self._db_config = {'host': self._config.get(f'plugins.{self.name}.database.host', 'localhost'), 'port': self._config.get(f'plugins.{self.name}.database.port', 5432), 'database': self._config.get(f'plugins.{self.name}.database.name', 'vcdb'), 'user': self._config.get(f'plugins.{self.name}.database.user', 'postgres'), 'password': self._config.get(f'plugins.{self.name}.database.password', '')}
        self._ui_config = {'max_filter_panels': self._config.get(f'plugins.{self.name}.ui.max_filter_panels', 5), 'default_page_size': self._config.get(f'plugins.{self.name}.ui.default_page_size', 100)}
        self._export_config = {'max_rows': self._config.get(f'plugins.{self.name}.export.max_rows', 0)}
        if self._logger:
    ...
    def on_ui_ready(self, ui_integration: UIIntegration) -> None:
        ...
        try:
            ...
            ui_integration.add_menu_action(self.name, vcdb_menu, 'Run Query', lambda: self._thread_manager.run_on_main_thread(self._run_query))
            ui_integration.add_menu_action(self.name, vcdb_menu, 'Refresh Filters', lambda: self._thread_manager.run_on_main_thread(self._refresh_filters))
            ui_integration.add_menu_action(self.name, vcdb_menu, 'Documentation', lambda: self._thread_manager.run_on_main_thread(self._open_documentation))
            ui_integration.add_menu_action(self.name, vcdb_menu, 'Configuration', lambda: self._thread_manager.run_on_main_thread(self._open_configuration))
            if not self._database_handler or not getattr(self._database_handler, '_initialized', False):
                ...
                try:
                    if not self._main_widget:
                        self._main_widget = VCdbExplorerWidget(self._database_handler, self._event_bus, self._thread_manager, self._logger, self._export_config, None)
                    icon = QIcon(self._icon_path) if self._icon_path else QIcon()
                    ui_integration.add_page(self.name, self._main_widget, f'plugin_{self.name}', icon, self.display_name or self.name, 'plugins')
                    if self._logger:
