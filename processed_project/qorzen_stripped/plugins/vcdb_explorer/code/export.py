...
class DataExporter:
    def __init__(self, logger: logging.Logger) -> None:
        self._logger = logger
    def export_csv(self, data: List[Dict[str, Any]], columns: List[str], column_map: Dict[str, str], file_path: str) -> None:
        try:
    ...
    def export_excel(self, data: List[Dict[str, Any]], columns: List[str], column_map: Dict[str, str], file_path: str, sheet_name: Optional[str]=None) -> None:
    ...
    def export_all_data(self, database_callback: Callable, filter_panels: List[Dict[str, List[int]]], columns: List[str], column_map: Dict[str, str], file_path: str, format_type: str, max_rows: int=0, table_filters: Optional[Dict[str, Any]]=None, sort_by: Optional[str]=None, sort_desc: bool=False, progress_callback: Optional[Callable[[int, int], None]]=None) -> int:
        try:
            _, total_count = database_callback(filter_panels=filter_panels, columns=columns, page=1, page_size=1, sort_by=sort_by, sort_desc=sort_desc, table_filters=table_filters)
            if max_rows == 0:
            ...
            if format_type == 'csv':
                with open(file_path, 'w', newline='', encoding='utf-8') as csv_file:
                    ...
                    for page in range(1, (total_to_export + batch_size - 1) // batch_size + 1):
                        ...
                        results, _ = database_callback(filter_panels=filter_panels, columns=columns, page=page, page_size=batch_size, sort_by=sort_by, sort_desc=sort_desc, table_filters=table_filters)
                        for row in results:
                            clean_row = {}
                            for col in columns:
                                value = row.get(col, '')
