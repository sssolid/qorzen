...
class FilterPanel(QGroupBox):
    ...
    def __init__(self, panel_id: str, database_handler: DatabaseHandler, event_bus: EventBusManager, logger: logging.Logger, parent: Optional[QWidget]=None) -> None:
        super().__init__(parent)
        ...
        self._event_bus.subscribe(event_type=VCdbEventType.filters_refreshed(), callback=self._on_filters_refreshed, subscriber_id=f'filter_panel_{panel_id}')
    ...
    def _refresh_filter_values(self, filter_type: str) -> None:
        ...
        try:
            ...
            values = self._database_handler.get_filter_values(filter_type if filter_type != 'year_range' else 'year', self._current_values, exclude_filters)
            self._filters[filter_type].set_available_values(values)
    ...
    def _on_filter_value_changed(self, filter_type: str, values: List[int]) -> None:
        ...
        self._event_bus.publish(event_type=VCdbEventType.filter_changed(), source='vcdb_explorer_filter_panel', payload={'panel_id': self._panel_id, 'filter_type': filter_type, 'values': values, 'current_filters': self._current_values.copy(), 'auto_populate': self._auto_populate_filters})
    @Slot(int)
            ...
            if self._auto_populate_filters:
                ...
                if self._current_values:
                    for filter_type, values in self._current_values.items():
                        ...
                        self._event_bus.publish(event_type=VCdbEventType.filter_changed(), source='vcdb_explorer_filter_panel', payload={'panel_id': self._panel_id, 'filter_type': filter_type, 'values': values, 'current_filters': self._current_values.copy(), 'auto_populate': True})
                else:
                    self._logger.debug('No current filter values, sending empty refresh')
                    self._event_bus.publish(event_type=VCdbEventType.filter_changed(), source='vcdb_explorer_filter_panel', payload={'panel_id': self._panel_id, 'filter_type': 'none', 'values': [], 'current_filters': {}, 'auto_populate': True})
            else:
...
class FilterPanelManager(QWidget):
    ...
    def __init__(self, database_handler: DatabaseHandler, event_bus: EventBusManager, logger: logging.Logger, max_panels: int=5, parent: Optional[QWidget]=None) -> None:
        super().__init__(parent)
        self._database_handler = database_handler
        self._event_bus = event_bus
        self._logger = logger
        ...
        self._event_bus.subscribe(event_type=VCdbEventType.filters_refreshed(), callback=self._on_filters_refreshed, subscriber_id='filter_panel_manager')
        self._add_panel()
    def __del__(self) -> None:
        try:
    ...
    def _add_panel(self) -> None:
        ...
        panel = FilterPanel(panel_id, self._database_handler, self._event_bus, self._logger)
