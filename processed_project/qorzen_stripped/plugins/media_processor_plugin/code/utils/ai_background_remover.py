...
class ModelType(str, Enum):
    ...
    MODNET = 'modnet'
class ModelDetails:
    MODELS = {ModelType.U2NET: {'name': 'U2Net', 'description': 'Original U2Net model for salient object detection', 'url': 'https://github.com/xuebinqin/U-2-Net/releases/download/U2NetHumanSeg/u2net_human_seg.pth', 'size': 173475292, 'hash': 'bc788c85e60baed816ffd6e2b2c6ae8a2b0afa57e335a14cc323cf1edf79abde'}, ModelType.U2NETP: {'name': 'U2Net-Lite', 'description': 'Lightweight version of U2Net (4.7MB)', 'url': 'https://github.com/xuebinqin/U-2-Net/releases/download/U2NetP/u2netp.pth', 'size': 4710256, 'hash': '8cb63ecf8d9a4b62be6abf8e5c8a52d9ebeb7fcb9c64d4a561b5682f2a734af6'}, ModelType.ISNET: {'name': 'ISNet', 'description': 'ISNet for high-quality human segmentation', 'url': 'https://github.com/xuebinqin/DIS/releases/download/1.1/isnet.pth', 'size': 176320856, 'hash': 'a1ca63c02e6c4e371a0a3e6a75223ca2e1b1755caa095f6b334f17f6b4292969'}, ModelType.MODNET: {'name': 'MODNet', 'description': 'MODNet for real-time portrait matting', 'url': 'https://github.com/ZHKKKe/MODNet/releases/download/v1.0/modnet_photographic_portrait_matting.pth', 'size': 24002228, 'hash': '815b64834ba6942c84c7b1c7ea36ebcbcb80b3e2c88b2d3eb25e7cc3fdb9453c'}}
class AIBackgroundRemover:
    def __init__(self, file_manager: Any, config_manager: Any, logger: Any):
    ...
    async def get_model_info(self, model_id: str) -> Dict[str, Any]:
        if model_id not in ModelDetails.MODELS:
            return {'name': 'Unknown Model', 'description': 'Unknown model type', 'downloaded': False, 'size': 0}
        model_info = ModelDetails.MODELS[model_id].copy()
        model_info['downloaded'] = await self.is_model_downloaded(model_id)
        return model_info
    async def download_model(self, model_id: str, progress_callback: Optional[Callable[[int, str], None]]=None) -> bool:
        if not self._models_dir:
        ...
        try:
            ...
            def _download_progress(count, block_size, total_size):
                ...
                if progress_callback:
                    ...
                    asyncio.run_coroutine_threadsafe(self._call_progress_on_main_thread(progress_callback, percent, message), asyncio.get_event_loop())
            urllib.request.urlretrieve(model_info['url'], temp_path, _download_progress)
            if progress_callback:
                await self._call_progress_on_main_thread(progress_callback, 95, 'Verifying download...')
            file_hash = self._compute_sha256(temp_path)
            if file_hash != model_info['hash']:
                if os.path.exists(temp_path):
                    os.remove(temp_path)
                raise BackgroundRemovalError(f"Downloaded file checksum mismatch: {file_hash} != {model_info['hash']}")
            if os.path.exists(model_path):
                os.remove(model_path)
            os.rename(temp_path, model_path)
            if progress_callback:
                await self._call_progress_on_main_thread(progress_callback, 100, 'Download complete')
            ...
            if os.path.exists(temp_path):
                os.remove(temp_path)
            if progress_callback:
                await self._call_progress_on_main_thread(progress_callback, 0, f'Error: {str(e)}')
            raise BackgroundRemovalError(f'Model download failed: {str(e)}')
    async def _call_progress_on_main_thread(self, callback: Callable[[int, str], None], percent: int, message: str) -> None:
        if hasattr(self, '_concurrency_manager'):
            await self._concurrency_manager.run_on_main_thread(lambda: callback(percent, message))
        else:
    ...
    async def remove_background(self, image: Image.Image, config: BackgroundRemovalConfig) -> Image.Image:
        if config.model_name not in ModelDetails.MODELS:
            raise BackgroundRemovalError(f'Unknown model: {config.model_name}, supported models: {list(ModelDetails.MODELS.keys())}')
        if not await self.is_model_downloaded(config.model_name):
            raise BackgroundRemovalError(f'Model {config.model_name} is not downloaded. Please download it first.')
        try:
    ...
    async def _process_with_u2net(self, image: Image.Image, model: Any, config: BackgroundRemovalConfig) -> Image.Image:
        if image.mode != 'RGB':
        ...
        return result
    async def _process_with_isnet(self, image: Image.Image, model: Any, config: BackgroundRemovalConfig) -> Image.Image:
        if image.mode != 'RGB':
            image = image.convert('RGB')
        ...
        result.putalpha(output)
        return result
    async def _process_with_modnet(self, image: Image.Image, model: Any, config: BackgroundRemovalConfig) -> Image.Image:
        if image.mode != 'RGB':
            image = image.convert('RGB')
