...
class AS400ConnectorPlugin(BasePlugin):
    ...
    def initialize(self, event_bus_manager: Any, logger_provider: Any, config_provider: Any, file_manager: Any=None, thread_manager: Any=None, database_manager: Any=None, security_manager: Any=None, **kwargs: Any) -> None:
        ...
        self._event_bus_manager.subscribe(event_type='config/changed', callback=self._on_config_changed, subscriber_id=f'{self.name}_config_subscriber')
        self._initialized = True
        self._logger.info(f'{self.name} plugin initialized')
        self._event_bus_manager.publish(event_type='plugin/initialized', source=self.name, payload={'plugin_name': self.name, 'version': self.version, 'has_ui': True})
    def on_ui_ready(self, ui_integration: UIIntegration) -> None:
        ...
        tab_index = ui_integration.add_tab(plugin_id=self.name, tab=self._tab, title='AS400 Connector')
        self._logger.debug(f'Added AS400 Connector tab at index {tab_index}')
        try:
            self._toolbar = ui_integration.add_toolbar(plugin_id=self.name, title='AS400 Connector')
        ...
        try:
            ...
            if tools_menu:
                self._menu = ui_integration.add_menu(plugin_id=self.name, title='AS400 Connector', parent_menu=tools_menu)
                action1 = ui_integration.add_menu_action(plugin_id=self.name, menu=self._menu, text='Connection...', callback=self._open_connection_dialog)
                action2 = ui_integration.add_menu_action(plugin_id=self.name, menu=self._menu, text='Manage Connections...', callback=self._manage_connections)
                self._menu.addSeparator()
                action3 = ui_integration.add_menu_action(plugin_id=self.name, menu=self._menu, text='Import Queries...', callback=self._import_queries)
                action4 = ui_integration.add_menu_action(plugin_id=self.name, menu=self._menu, text='Export Queries...', callback=self._export_queries)
    ...
    def shutdown(self) -> None:
        ...
        if self._event_bus_manager:
            ...
            self._event_bus_manager.publish(event_type='plugin/shutdown', source=self.name, payload={'plugin_name': self.name}, synchronous=True)
        self._initialized = False
        self._logger.info(f'{self.name} plugin shut down')
    def status(self) -> Dict[str, Any]:
        return {'name': self.name, 'version': self.version, 'initialized': self._initialized, 'has_ui': True, 'ui_components': {'tab_added': self._tab is not None, 'tab_index': self._tab_index, 'menu_items_count': len(self._menu_items)}, 'subscriptions': ['ui/ready', 'config/changed']}