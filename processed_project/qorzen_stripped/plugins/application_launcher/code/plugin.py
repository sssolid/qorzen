...
class ArgumentInputWidget(QWidget):
    ...
    def __init__(self, arg_config: ArgumentConfig, parent: Optional[QWidget]=None) -> None:
...
class ApplicationConfigDialog(QDialog):
    def __init__(self, app_config: Optional[ApplicationConfig]=None, parent: Optional[QWidget]=None) -> None:
        super().__init__(parent)
        self.setWindowTitle('Configure Application')
        self.setMinimumWidth(600)
        self.setMinimumHeight(500)
        self.app_config = app_config or ApplicationConfig(id=f'app_{os.urandom(4).hex()}', name='New Application', executable_path='')
    ...
    def _init_ui(self) -> None:
        ...
        self.args_table.setHorizontalHeaderLabels(['Name', 'Type', 'Required', 'Description', 'Value'])
    ...
    def get_config(self) -> ApplicationConfig:
        ...
        for row in range(self.args_table.rowCount()):
            ...
            if original_arg:
                arg = ArgumentConfig(name=name, arg_type=arg_type, value=value, description=description, required=required, file_filter=original_arg.file_filter, prefix=original_arg.prefix)
            else:
                arg = ArgumentConfig(name=name, arg_type=arg_type, value=value, description=description, required=required, file_filter='', prefix='')
    ...
    def _browse_working_dir(self) -> None:
        dir_path = QFileDialog.getExistingDirectory(self, 'Select Working Directory', '', QFileDialog.ShowDirsOnly)
        if dir_path:
            self.working_dir_edit.setText(dir_path)
    def _browse_icon(self) -> None:
        file_filter = 'Image Files (*.png *.jpg *.jpeg *.ico);;All Files (*.*)'
        file_path, _ = QFileDialog.getOpenFileName(self, 'Select Icon', '', file_filter)
        if file_path:
    ...
    def _edit_argument(self) -> None:
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            for i, arg in enumerate(self.app_config.arguments):
                ...
                self.app_config.arguments.append(ArgumentConfig(name=name_edit.text(), arg_type=ArgumentType(type_combo.currentText()), value=value_edit.text(), description=desc_edit.text(), required=req_check.isChecked(), file_filter=filter_edit.text(), prefix=prefix_edit.text()))
...
class OutputFilesWidget(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self._instructions_label = QLabel('Note: Output directories are automatically detected from output file locations and displayed in the dropdown above.')
        ...
        self.files_table.setHorizontalHeaderLabels(['Filename', 'Size', 'Modified', 'Path', 'Actions'])
    ...
    def _open_file(self, file_path: str) -> None:
        ...
        if QDesktopServices.openUrl(url):
            ...
            QMessageBox.warning(self, 'Open Failed', f'Failed to open file: {file_path}')
    def _open_file_with(self, file_path: str, file_type: str) -> None:
        ...
        try:
            ...
            QMessageBox.warning(self, 'Open Failed', f'Failed to open file with specified application: {str(e)}')
            self._open_file(file_path)
    def _show_in_folder(self, file_path: str) -> None:
        directory = os.path.dirname(file_path)
        url = QUrl.fromLocalFile(directory)
        if not QDesktopServices.openUrl(url):
            QMessageBox.warning(self, 'Show in Folder Failed', f'Failed to open folder: {directory}')
    def _open_selected_directory(self) -> None:
        directory = self._output_dirs_combo.currentText()
        if directory:
            url = QUrl.fromLocalFile(directory)
            if not QDesktopServices.openUrl(url):
                QMessageBox.warning(self, 'Open Directory Failed', f'Failed to open directory: {directory}')
class ApplicationRunner(QObject):
    ...
    def __init__(self, concurrency_manager: ConcurrencyManager, file_manager: FileManager, parent: Optional[QObject]=None) -> None:
        super().__init__(parent)
    ...
    def run_application(self, app_config: ApplicationConfig, arg_values: Dict[str, str]) -> None:
        ...
        for arg_config in app_config.arguments:
            arg_copy = ArgumentConfig(name=arg_config.name, arg_type=arg_config.arg_type, value=arg_config.value, description=arg_config.description, required=arg_config.required, file_filter=arg_config.file_filter, prefix=arg_config.prefix)
        ...
        import time
        self._output = ProcessOutput(status=ProcessStatus.RUNNING, stdout='', stderr='', start_time=time.time(), command_line=command_line)
    ...
    def _process_error(self, error: QProcess.ProcessError) -> None:
        error_messages = {QProcess.FailedToStart: 'Failed to start process. Make sure the executable exists and you have permission to run it.', QProcess.Crashed: 'Process crashed.', QProcess.Timedout: 'Process timed out.', QProcess.WriteError: 'Error writing to process.', QProcess.ReadError: 'Error reading from process.', QProcess.UnknownError: 'Unknown process error.'}
...
class ApplicationCard(QWidget):
    ...
    def __init__(self, app_config: ApplicationConfig, parent: Optional[QWidget]=None) -> None:
        super().__init__(parent)
...
class ApplicationRunDialog(QDialog):
    def __init__(self, app_config: ApplicationConfig, app_runner: ApplicationRunner, parent: Optional[QWidget]=None) -> None:
        super().__init__(parent)
        self.app_config = app_config
    ...
    def _init_ui(self) -> None:
        ...
        for arg_config in self.app_config.arguments:
            ...
            arg_input.valueChanged.connect(lambda value, name=arg_config.name: self._on_arg_value_changed(name, value))
            ...
            if arg_config.arg_type in (ArgumentType.FILE_INPUT, ArgumentType.FILE_OUTPUT, ArgumentType.DIRECTORY):
                ...
                open_btn.clicked.connect(lambda checked=False, name=arg_config.name: self._open_path(self.arg_values.get(name, arg_config.value)))
    ...
    def _open_path(self, path: str) -> None:
        ...
        if not os.path.exists(resolved_path):
            QMessageBox.warning(self, 'Path Not Found', f'The path does not exist: {resolved_path}')
            return
        url = QUrl.fromLocalFile(resolved_path)
        if not QDesktopServices.openUrl(url):
            QMessageBox.warning(self, 'Open Failed', f'Failed to open: {resolved_path}')
    def _on_run_clicked(self) -> None:
        ...
        if missing_args:
            QMessageBox.warning(self, 'Missing Arguments', f"The following required arguments are missing: {', '.join(missing_args)}")
            return
...
class ApplicationLauncherWidget(QWidget):
    def __init__(self, event_bus_manager: EventBusManager, concurrency_manager: ConcurrencyManager, task_manager: TaskManager, file_manager: FileManager, logger: logging.Logger, parent: Optional[QWidget]=None) -> None:
        super().__init__(parent)
            ...
            try:
                ...
                for app_id, app_data in apps_dict.items():
                    ...
                    for arg_data in app_data.get('arguments', []):
                        arguments.append(ArgumentConfig(name=arg_data.get('name', ''), arg_type=ArgumentType(arg_data.get('arg_type', ArgumentType.STATIC)), value=arg_data.get('value', ''), description=arg_data.get('description', ''), required=arg_data.get('required', False), file_filter=arg_data.get('file_filter', ''), prefix=arg_data.get('prefix', '')))
                    app_config = ApplicationConfig(id=app_id, name=app_data.get('name', 'Unknown'), executable_path=app_data.get('executable_path', ''), working_directory=app_data.get('working_directory'), arguments=arguments, environment_variables=app_data.get('environment_variables', {}), show_console_output=app_data.get('show_console_output', True), category=app_data.get('category', 'General'), description=app_data.get('description', ''), icon_path=app_data.get('icon_path'), output_patterns=app_data.get('output_patterns', []), auto_open_output=app_data.get('auto_open_output', False))
                    self._app_configs[app_id] = app_config
                await self._concurrency_manager.run_on_main_thread(self._update_applications_ui)
        ...
        try:
            ...
            for app_id, app_config in self._app_configs.items():
                ...
                for arg in app_config.arguments:
                    arguments.append({'name': arg.name, 'arg_type': arg.arg_type, 'value': arg.value, 'description': arg.description, 'required': arg.required, 'file_filter': arg.file_filter, 'prefix': arg.prefix})
                apps_dict[app_id] = {'name': app_config.name, 'executable_path': app_config.executable_path, 'working_directory': app_config.working_directory, 'arguments': arguments, 'environment_variables': app_config.environment_variables, 'show_console_output': app_config.show_console_output, 'category': app_config.category, 'description': app_config.description, 'icon_path': app_config.icon_path, 'output_patterns': app_config.output_patterns, 'auto_open_output': app_config.auto_open_output}
            import json
    ...
    def _update_applications_ui(self) -> None:
        ...
        filtered_apps = []
        for app_id, app_config in self._app_configs.items():
            if search_text and (not (search_text in app_config.name.lower() or search_text in app_config.description.lower())):
                continue
    ...
    def _on_delete_app(self, app_config: ApplicationConfig) -> None:
        reply = QMessageBox.question(self, 'Confirm Delete', f"Are you sure you want to delete '{app_config.name}'?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
...
class ApplicationLauncherPlugin(BasePlugin):
    ...
    async def initialize(self, application_core: Any, **kwargs: Any) -> None:
        ...
        await self._event_bus_manager.subscribe(event_type='application_launcher:log_message', callback=self._on_log_message, subscriber_id='application_launcher_plugin')
    ...
    async def on_ui_ready(self, ui_integration: UIIntegration) -> None:
        ...
        try:
            await set_plugin_state(self.name, PluginLifecycleState.UI_READY)
            await ui_integration.add_menu_item(plugin_id=self.plugin_id, parent_menu='Tools', title='Application Launcher', callback=lambda: asyncio.create_task(self._open_application_launcher()), icon=self._icon_path)
            if not self._main_widget:
                self._main_widget = ApplicationLauncherWidget(self._event_bus_manager, self._concurrency_manager, self._task_manager, self._file_manager, self._logger, None)
