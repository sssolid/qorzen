...
class CounterWidget(AsyncQWidget):
    ...
    def _update_display(self) -> None:
        self.run_async_task(self._plugin._get_counter, task_id='get_counter', on_result=self._on_counter_updated)
    ...
    def _on_counter_updated(self, counter_value: int) -> None:
        ...
        if prev_value != current_value:
            ...
            history_item = QListWidgetItem(f'{current_time}: Counter updated to {counter_value}')
    ...
    def _on_increment_clicked(self) -> None:
        ...
        self.run_async_task(self._plugin._increment_counter, 1, task_id='increment_counter', on_result=self._on_increment_done, on_error=self._on_task_error, on_finished=lambda: self._increment_button.setEnabled(True))
    def _on_increment_by_10_clicked(self) -> None:
        self._increment_by_10_button.setEnabled(False)
        self.run_async_task(self._plugin._increment_counter, 10, task_id='increment_by_10', on_result=self._on_increment_done, on_error=self._on_task_error, on_finished=lambda: self._increment_by_10_button.setEnabled(True))
    ...
    def _on_task_error(self, error_msg: str, traceback_str: str) -> None:
        if self._plugin._logger:
            self._plugin._logger.error(f'Error in counter widget: {error_msg}', extra={'traceback': traceback_str})
        current_time = datetime.now().strftime('%H:%M:%S')
        history_item = QListWidgetItem(f'{current_time}: Error - {error_msg}')
...
class SampleAsyncPlugin(BasePlugin):
    ...
    async def initialize(self, application_core: Any, **kwargs: Any) -> None:
        try:
            ...
            if self._task_manager:
                self._timer_task_id = await self._task_manager.submit_async_task(func=self._timer_loop, name='plugin_timer_loop', plugin_id=self.plugin_id, category=TaskCategory.PLUGIN)
            ...
            if self._event_bus_manager:
                sub_id = await self._event_bus_manager.subscribe(event_type=f'plugin/{self.name}/counter_updated', callback=self._on_counter_updated, subscriber_id=f'{self.name}_counter_listener')
    ...
    async def on_ui_ready(self, ui_integration: Any) -> None:
        try:
            ...
            try:
                self._menu_item_id = await ui_integration.add_menu_item(plugin_id=self.plugin_id, title='Sample Plugin', callback=self._on_menu_clicked, parent_menu='Plugins')
    ...
    async def _setup_counter_page(self, ui_integration: Any) -> None:
        ...
        try:
            ...
            self._counter_page_id = await ui_integration.add_page(plugin_id=self.plugin_id, page_component=self._counter_widget, title='Sample Plugin Counter', icon=':/ui_icons/trending-up.svg')
            if self._logger:
    ...
    async def _timer_loop(self) -> None:
        try:
            while True:
                ...
                if self._event_bus_manager:
                    try:
                        await self._event_bus_manager.publish(event_type=f'plugin/{self.name}/counter_updated', source=self.name, payload={'counter': self._counter})
                    except Exception as e:
                        if self._logger:
            ...
            if not self._shutdown and self._task_manager:
                try:
                    new_task_id = await self._task_manager.submit_async_task(func=self._timer_loop, name='plugin_timer_loop', plugin_id=self.plugin_id, category=TaskCategory.PLUGIN)
                    if new_task_id:
    ...
    async def _increment_counter(self, amount: int=1) -> int:
        ...
        if self._event_bus_manager:
            try:
                await self._event_bus_manager.publish(event_type=f'plugin/{self.name}/counter_updated', source=self.name, payload={'counter': self._counter, 'increment': amount})
            except Exception as e:
                if self._logger:
    ...
    async def _on_menu_clicked(self) -> None:
        ...
        try:
            ...
            if ui:
                await ui.show_notification(plugin_id=self.plugin_id, message=f'Counter incremented to {self._counter}', title='Sample Plugin', notification_type='info')
