...
class PluginCapability(str, enum.Enum):
    ...
    def get_description(cls, capability: PluginCapability) -> str:
        descriptions = {cls.CONFIG_READ: 'Read application configuration settings', cls.CONFIG_WRITE: 'Modify application configuration settings', cls.UI_EXTEND: 'Add elements to the user interface', cls.EVENT_SUBSCRIBE: 'Subscribe to application events', cls.EVENT_PUBLISH: 'Publish events to the application event bus', cls.FILE_READ: 'Read files from the file system', cls.FILE_WRITE: 'Write files to the file system', cls.NETWORK_CONNECT: 'Connect to external services over the network', cls.DATABASE_READ: 'Read data from the application database', cls.DATABASE_WRITE: 'Write data to the application database', cls.SYSTEM_EXEC: 'Execute system commands (high privilege)', cls.SYSTEM_MONITOR: 'Monitor system resources and performance', cls.PLUGIN_COMMUNICATE: 'Communicate with other plugins'}
...
class PluginAuthor(pydantic.BaseModel):
    ...
    def validate_url(cls, v: Optional[str]) -> Optional[str]:
        if v is None:
            return None
        url_regex = "^https?://[a-zA-Z0-9.-]+\\.[a-zA-Z]{2,}(/[a-zA-Z0-9._~:/?#[\\]@!$&'()*+,;=%-]*)?$"
...
class PluginDependency(pydantic.BaseModel):
    ...
    def validate_version(cls, v: str) -> str:
        semver_regex = '^(=|>=|<=|>|<|~=|!=|\\^)?(\\d+)\\.(\\d+)\\.(\\d+)(-[0-9a-zA-Z.-]+)?(\\+[0-9a-zA-Z.-]+)?$'
...
class PluginManifest(pydantic.BaseModel):
    ...
    def validate_license(cls, v: str) -> str:
        common_licenses = {'MIT', 'Apache-2.0', 'GPL-3.0', 'GPL-2.0', 'LGPL-3.0', 'LGPL-2.1', 'BSD-3-Clause', 'BSD-2-Clause', 'MPL-2.0', 'AGPL-3.0', 'Unlicense', 'proprietary', 'custom'}
    ...
    def satisfies_dependency(self, dependency: PluginDependency) -> bool:
        ...
        try:
            ...
            if operator == '=':
                ...
                return plugin_version >= dependency_version and plugin_version.major == dependency_version.major and (plugin_version.minor == dependency_version.minor)
            elif operator == '^':
                return plugin_version >= dependency_version and plugin_version.major == dependency_version.major
            else:
                return False
        except ValueError:
            return False
