    ...
    def disable_plugin(self, plugin_name: str) -> bool:
        ...
        if result:
            try:
                ...
                pass
        return result
    def install_plugin(self, package_path: Union[str, Path], force: bool=False, skip_verification: bool=False, enable: bool=True, resolve_dependencies: bool=True, install_dependencies: bool=True) -> InstalledPlugin:
        with self._lock:
            ...
            try:
                if PluginLifecycleHook.PRE_INSTALL in manifest.lifecycle_hooks:
                    with tempfile.TemporaryDirectory() as temp_dir:
                        ...
                        try:
                            execute_hook(hook=PluginLifecycleHook.PRE_INSTALL, plugin_name=plugin_name, manifest=manifest, context={'package_path': str(package_path), 'force': force, 'skip_verification': skip_verification, 'enable': enable, 'resolve_dependencies': resolve_dependencies, 'install_dependencies': install_dependencies, 'plugins_dir': str(self.plugins_dir)})
                        finally:
            ...
            try:
                if PluginLifecycleHook.POST_INSTALL in manifest.lifecycle_hooks:
                    execute_hook(hook=PluginLifecycleHook.POST_INSTALL, plugin_name=plugin_name, manifest=manifest, context={'package_path': str(package_path), 'force': force, 'skip_verification': skip_verification, 'enable': enable, 'resolve_dependencies': resolve_dependencies, 'install_dependencies': install_dependencies, 'plugins_dir': str(self.plugins_dir), 'install_path': str(installed_plugin.install_path)})
            except LifecycleHookError as e:
        ...
        with self._lock:
            ...
            try:
                if PluginLifecycleHook.PRE_UNINSTALL in manifest.lifecycle_hooks:
                    execute_hook(hook=PluginLifecycleHook.PRE_UNINSTALL, plugin_name=plugin_name, manifest=manifest, context={'keep_data': keep_data, 'check_dependents': check_dependents, 'plugins_dir': str(self.plugins_dir), 'install_path': str(installed_plugin.install_path)})
            except LifecycleHookError as e:
            ...
            if result:
                ...
                try:
                    if PluginLifecycleHook.POST_UNINSTALL in manifest.lifecycle_hooks:
                        execute_hook(hook=PluginLifecycleHook.POST_UNINSTALL, plugin_name=plugin_name, manifest=manifest, context={'keep_data': keep_data, 'check_dependents': check_dependents, 'plugins_dir': str(self.plugins_dir), 'success': result})
                except LifecycleHookError as e:
                    self.log(f'Post-uninstall hook failed: {str(e)}', 'warning')
            return result
    def update_plugin(self, package_path: Union[str, Path], skip_verification: bool=False, resolve_dependencies: bool=True, install_dependencies: bool=True) -> InstalledPlugin:
        with self._lock:
            package = PluginPackage.load(package_path)
            if not package.manifest:
                raise PluginInstallationError('Package has no manifest')
            ...
            try:
                if PluginLifecycleHook.PRE_UPDATE in installed_plugin.manifest.lifecycle_hooks:
                    execute_hook(hook=PluginLifecycleHook.PRE_UPDATE, plugin_name=plugin_name, manifest=installed_plugin.manifest, context={'package_path': str(package_path), 'skip_verification': skip_verification, 'resolve_dependencies': resolve_dependencies, 'install_dependencies': install_dependencies, 'plugins_dir': str(self.plugins_dir), 'install_path': str(installed_plugin.install_path), 'current_version': current_version, 'new_version': new_version})
            except LifecycleHookError as e:
            ...
            updated_plugin = self.install_plugin(package_path=package_path, force=True, skip_verification=skip_verification, enable=installed_plugin.enabled, resolve_dependencies=resolve_dependencies, install_dependencies=install_dependencies)
            try:
                if PluginLifecycleHook.POST_UPDATE in manifest.lifecycle_hooks:
                    execute_hook(hook=PluginLifecycleHook.POST_UPDATE, plugin_name=plugin_name, manifest=manifest, context={'package_path': str(package_path), 'skip_verification': skip_verification, 'resolve_dependencies': resolve_dependencies, 'install_dependencies': install_dependencies, 'plugins_dir': str(self.plugins_dir), 'install_path': str(updated_plugin.install_path), 'current_version': current_version, 'new_version': new_version, 'success': True})
            except LifecycleHookError as e:
