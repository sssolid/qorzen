...
def create_command(args: argparse.Namespace) -> int:
    try:
        ...
        plugin_dir = create_plugin_template(output_dir=output_dir, plugin_name=args.name, display_name=args.display_name, description=args.description, author_name=args.author_name, author_email=args.author_email, author_url=args.author_url, version=args.version, license=args.license, force=args.force)
...
def package_command(args: argparse.Namespace) -> int:
    try:
        ...
        package_path = package_plugin(plugin_dir=plugin_dir, output_path=output_path, format=format_type, signing_key=signing_key, include_patterns=args.include, exclude_patterns=args.exclude)
        ...
        if package_path.is_dir():
            ...
            if package.format == PackageFormat.DIRECTORY:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    new_package = PluginPackage.create(source_dir=temp_path, output_path=package_path, format=package.format)
...
def test_command(args: argparse.Namespace) -> int:
    try:
        plugin_dir = Path(args.plugin_dir)
        success = test_plugin(plugin_dir=plugin_dir, mock_env=not args.no_mock, test_pattern=args.pattern)
...
def install_command(args: argparse.Namespace) -> int:
    try:
        ...
        try:
            plugin = installer.install_plugin(package_path=package_path, force=args.force, skip_verification=args.skip_verification, enable=not args.disable)
...
def uninstall_command(args: argparse.Namespace) -> int:
    try:
        ...
        installer = PluginInstaller(plugins_dir)
        try:
            success = installer.uninstall_plugin(plugin_name=args.name, keep_data=args.keep_data)
            if success:
...
def main(args: Optional[List[str]]=None) -> int:
    parser = argparse.ArgumentParser(description='Qorzen Plugin System CLI', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    package_parser.add_argument('--include', action='append', default=None, help='Include pattern (can be specified multiple times)')
    package_parser.add_argument('--exclude', action='append', default=None, help='Exclude pattern (can be specified multiple times)')
