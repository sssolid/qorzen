...
class LifecycleManager:
    ...
    def register_ui_integration(self, plugin_name: str, ui_integration: UIIntegration, main_window: Optional[Any]=None) -> None:
        with self._ui_integrations_lock:
    ...
    def execute_hook(self, hook: PluginLifecycleHook, plugin_name: str, manifest: PluginManifest, plugin_instance: Optional[Any]=None, context: Optional[Dict[str, Any]]=None) -> Any:
        context = context or {}
        ...
        try:
            ...
            if hook in ui_hooks and self._thread_manager and (not self._thread_manager.is_main_thread()):
                return self._thread_manager.execute_on_main_thread_sync(self.execute_hook, hook, plugin_name, manifest, plugin_instance, context)
...
def execute_hook(hook: PluginLifecycleHook, plugin_name: str, manifest: PluginManifest, plugin_instance: Optional[Any]=None, *, context: Optional[Dict[str, Any]]=None, **kwargs: Any) -> Any:
    if context is None:
        context = kwargs
    else:
        context.update(kwargs)
    return get_lifecycle_manager().execute_hook(hook=hook, plugin_name=plugin_name, manifest=manifest, plugin_instance=plugin_instance, context=context)
