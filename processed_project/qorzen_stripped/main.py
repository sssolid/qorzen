...
async def start_ui(args: argparse.Namespace) -> int:
    try:
        ...
        async def wait_for_shutdown():
            ...
            done, pending = await asyncio.wait([asyncio.create_task(app_shutdown_signal.wait()), core_shutdown_task], return_when=asyncio.FIRST_COMPLETED)
            shutdown_signal.set()
            for task in pending:
                task.cancel()
                try:
...
async def handle_build_command(args: argparse.Namespace) -> int:
    try:
        ...
        build_args = [f'--platform={args.platform}', f'--build-type={args.type}', f'--output-dir={args.output_dir}']
        if args.clean:
